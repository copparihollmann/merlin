#executable_target_embedded_elf_x86_64 = #hal.executable.target<"llvm-cpu", "embedded-elf-x86_64", {cpu = "znver3", cpu_features = "+prfchw,-cldemote,+avx,+aes,+sahf,+pclmul,-xop,+crc32,-amx-fp8,+xsaves,-avx512fp16,-usermsr,-sm4,-egpr,+sse4.1,-avx512ifma,+xsave,+sse4.2,-tsxldtrk,-sm3,-ptwrite,-widekl,-movrs,+invpcid,+64bit,+xsavec,-avx10.1-512,-avx512vpopcntdq,+cmov,-avx512vp2intersect,-avx512cd,+movbe,-avxvnniint8,-ccmp,-amx-int8,-kl,-avx10.1-256,-sha512,-avxvnni,-rtm,+adx,+avx2,-hreset,-movdiri,-serialize,+vpclmulqdq,-avx512vl,-uintr,-cf,+clflushopt,-raoint,-cmpccxadd,+bmi,-amx-tile,+sse,-avx10.2-256,-gfni,-avxvnniint16,-amx-fp16,-zu,-ndd,+xsaveopt,+rdrnd,-avx512f,-amx-bf16,-avx512bf16,-avx512vnni,-push2pop2,+cx8,-avx512bw,+sse3,+pku,-nf,-amx-tf32,-amx-avx512,+fsgsbase,+clzero,+mwaitx,-lwp,+lzcnt,+sha,-movdir64b,-ppx,+wbnoinvd,-enqcmd,-amx-transpose,-avx10.2-512,-avxneconvert,-tbm,-pconfig,-amx-complex,+ssse3,+cx16,+bmi2,+fma,+popcnt,-avxifma,+f16c,-avx512bitalg,+rdpru,+clwb,+mmx,+sse2,+rdseed,-avx512vbmi2,-prefetchi,-amx-movrs,+rdpid,-fma4,-avx512vbmi,+shstk,+vaes,-waitpkg,-sgx,+fxsr,-avx512dq,+sse4a", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_triple = "x86_64-unknown-unknown-eabi-elf"}>
#map = affine_map<() -> ()>
#map1 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 * 4 + d4, d2 * 4 + d5)>
#map2 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>
#map3 = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>
#map4 = affine_map<(d0, d1, d2) -> (d0, d1, d2)>
#map5 = affine_map<(d0, d1, d2) -> (d0)>
#map6 = affine_map<(d0, d1, d2, d3, d4) -> (d0, d1 * 2 + d3, d2 * 2 + d4)>
#map7 = affine_map<(d0, d1, d2, d3, d4) -> (d3, d4)>
#map8 = affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d2)>
#map9 = affine_map<(d0, d1) -> (d0, d1)>
#map10 = affine_map<(d0) -> (d0)>
#map11 = affine_map<(d0) -> ()>
#map12 = affine_map<(d0, d1) -> (d1)>
#map13 = affine_map<(d0, d1) -> (d0)>
#device_target_local = #hal.device.target<"local", [#executable_target_embedded_elf_x86_64]> : !hal.device
module @integrated_robot_application attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  util.global private @__device_0 = #device_target_local
  util.func public @main$async(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view, %arg3: !hal.buffer_view, %arg4: !hal.fence, %arg5: !hal.fence, %arg6: !hal.fence, %arg7: !hal.fence) -> (!hal.buffer_view, !hal.buffer_view) attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "async func @main$async(%input0: !hal.buffer_view, %input1: !hal.buffer_view, %input2: !hal.buffer_view, %input3: !hal.buffer_view, %input4: !hal.fence, %input5: !hal.fence) -> (%output0: !hal.buffer_view, %output1: !hal.buffer_view)", iree.abi.model = "coarse-fences"}} {
    %0:2 = util.call @_main$async(%arg0, %arg1, %arg2, %arg3, %arg4, %arg5) : (!hal.buffer_view, !hal.buffer_view, !hal.buffer_view, !hal.buffer_view, !hal.fence, !hal.fence) -> (!hal.buffer_view, !hal.buffer_view)
    hal.fence.signal<%arg7 : !hal.fence>
    util.return %0#0, %0#1 : !hal.buffer_view, !hal.buffer_view
  }
  util.func private @_main$async(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view, %arg3: !hal.buffer_view, %arg4: !hal.fence, %arg5: !hal.fence) -> (!hal.buffer_view, !hal.buffer_view) attributes {inlining_policy = #util.inline.never} {
    %c10 = arith.constant 10 : index
    %c2 = arith.constant 2 : index
    %c0 = arith.constant 0 : index
    %cst = arith.constant 9.99999993E-9 : f32
    %c1 = arith.constant 1 : index
    %cst_0 = arith.constant 0.000000e+00 : f32
    %cst_1 = arith.constant dense<""> : tensor<4x3x4x4xf32>
    %cst_2 = arith.constant dense<""> : tensor<1x256x8x1xf32>
    %0 = hal.tensor.import wait(%arg4) => %arg0 : !hal.buffer_view -> tensor<1x3x64x64xf32>
    %1 = hal.tensor.import wait(%arg4) => %arg1 : !hal.buffer_view -> tensor<3xf32>
    %2 = hal.tensor.import wait(%arg4) => %arg2 : !hal.buffer_view -> tensor<2xf32>
    %3 = hal.tensor.import wait(%arg4) => %arg3 : !hal.buffer_view -> tensor<2xf32>
    %4:3 = flow.dispatch.workgroups(%3, %1) : (tensor<2xf32>, tensor<3xf32>) -> (tensor<f32>, tensor<f32>, tensor<f32>) =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<3xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>, %arg9: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>, %arg10: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
      %cst_3 = arith.constant 9.99999974E-5 : f32
      %cst_4 = arith.constant 2.000000e+00 : f32
      %cst_5 = arith.constant 5.000000e-01 : f32
      %c0_6 = arith.constant 0 : index
      %c1_7 = arith.constant 1 : index
      %c2_8 = arith.constant 2 : index
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
      %28 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [3], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3xf32>> -> tensor<3xf32>
      %29 = tensor.empty() : tensor<f32>
      %30 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%29 : tensor<f32>) {
      ^bb0(%out: f32):
        %extracted = tensor.extract %27[%c0_6] : tensor<2xf32>
        %33 = arith.mulf %extracted, %cst_3 : f32
        %extracted_9 = tensor.extract %27[%c1_7] : tensor<2xf32>
        %34 = arith.mulf %extracted_9, %cst_3 : f32
        %35 = arith.addf %33, %34 : f32
        %36 = arith.divf %35, %cst_4 : f32
        %37 = arith.subf %34, %33 : f32
        %38 = arith.divf %37, %cst_5 : f32
        %extracted_10 = tensor.extract %28[%c2_8] : tensor<3xf32>
        %39 = arith.divf %38, %cst_4 : f32
        %40 = arith.addf %extracted_10, %39 : f32
        %41 = math.sin %40 : f32
        %42 = arith.mulf %36, %41 : f32
        %extracted_11 = tensor.extract %28[%c1_7] : tensor<3xf32>
        %43 = arith.addf %extracted_11, %42 : f32
        linalg.yield %43 : f32
      } -> tensor<f32>
      %31 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%29 : tensor<f32>) {
      ^bb0(%out: f32):
        %extracted = tensor.extract %27[%c0_6] : tensor<2xf32>
        %33 = arith.mulf %extracted, %cst_3 : f32
        %extracted_9 = tensor.extract %27[%c1_7] : tensor<2xf32>
        %34 = arith.mulf %extracted_9, %cst_3 : f32
        %35 = arith.addf %33, %34 : f32
        %36 = arith.divf %35, %cst_4 : f32
        %37 = arith.subf %34, %33 : f32
        %38 = arith.divf %37, %cst_5 : f32
        %extracted_10 = tensor.extract %28[%c2_8] : tensor<3xf32>
        %39 = arith.divf %38, %cst_4 : f32
        %40 = arith.addf %extracted_10, %39 : f32
        %41 = math.cos %40 : f32
        %42 = arith.mulf %36, %41 : f32
        %extracted_11 = tensor.extract %28[%c0_6] : tensor<3xf32>
        %43 = arith.addf %extracted_11, %42 : f32
        linalg.yield %43 : f32
      } -> tensor<f32>
      %32 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%29 : tensor<f32>) {
      ^bb0(%out: f32):
        %extracted = tensor.extract %27[%c0_6] : tensor<2xf32>
        %33 = arith.mulf %extracted, %cst_3 : f32
        %extracted_9 = tensor.extract %27[%c1_7] : tensor<2xf32>
        %34 = arith.mulf %extracted_9, %cst_3 : f32
        %35 = arith.subf %34, %33 : f32
        %36 = arith.divf %35, %cst_5 : f32
        %extracted_10 = tensor.extract %28[%c2_8] : tensor<3xf32>
        %37 = arith.addf %extracted_10, %36 : f32
        linalg.yield %37 : f32
      } -> tensor<f32>
      iree_tensor_ext.dispatch.tensor.store %32, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
      iree_tensor_ext.dispatch.tensor.store %31, %arg9, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
      iree_tensor_ext.dispatch.tensor.store %30, %arg10, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
      flow.return
    } count() -> (index, index, index) {
      %c1_3 = arith.constant 1 : index
      flow.return %c1_3, %c1_3, %c1_3 : index, index, index
    }
    %5 = flow.tensor.reshape %4#1 : tensor<f32> -> tensor<1xf32>
    %6 = flow.tensor.reshape %4#2 : tensor<f32> -> tensor<1xf32>
    %7 = flow.tensor.reshape %4#0 : tensor<f32> -> tensor<1xf32>
    %8 = tensor.empty() : tensor<3xf32>
    %9 = flow.tensor.update %5, %8[%c0] : tensor<1xf32> -> %8 as tensor<3xf32>
    %10 = flow.tensor.update %6, %9[%c1] : tensor<1xf32> -> %9 as tensor<3xf32>
    %11 = flow.tensor.update %7, %10[%c2] : tensor<1xf32> -> %10 as tensor<3xf32>
    %12 = flow.tensor.splat %cst_0 : tensor<1x3x66x66xf32>
    %13 = flow.tensor.reshape %0 : tensor<1x3x64x64xf32> -> tensor<3x64x64xf32>
    %14 = flow.dispatch.workgroups(%13, %12) : (tensor<3x64x64xf32>, tensor<1x3x66x66xf32>) -> %12 =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x64x64xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readwrite:tensor<1x3x66x66xf32>>) {
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0, 0, 0], sizes = [3, 64, 64], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x64x64xf32>> -> tensor<3x64x64xf32>
      iree_tensor_ext.dispatch.tensor.store %27, %arg7, offsets = [0, 0, 1, 1], sizes = [1, 3, 64, 64], strides = [1, 1, 1, 1] : tensor<3x64x64xf32> -> !iree_tensor_ext.dispatch.tensor<readwrite:tensor<1x3x66x66xf32>>
      flow.return
    } count() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      flow.return %x, %y, %z : index, index, index
    }
    %15 = flow.tensor.reshape %14 : tensor<1x3x66x66xf32> -> tensor<3x66x66xf32>
    %16 = flow.dispatch.workgroups(%15, %cst_1) : (tensor<3x66x66xf32>, tensor<4x3x4x4xf32>) -> tensor<4x16x16xf32> =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x66x66xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x3x4x4xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x16x16xf32>>) {
      %cst_3 = arith.constant 0.000000e+00 : f32
      %cst_4 = arith.constant dense<[0.0602946617, 0.0679099783, 0.0262253601, 2.52176687E-4]> : tensor<4xf32>
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0, 0, 0], sizes = [3, 66, 66], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3x66x66xf32>> -> tensor<3x66x66xf32>
      %28 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0, 0, 0], sizes = [4, 3, 4, 4], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x3x4x4xf32>> -> tensor<4x3x4x4xf32>
      %29 = tensor.empty() : tensor<4x16x16xf32>
      %30 = linalg.fill ins(%cst_3 : f32) outs(%29 : tensor<4x16x16xf32>) -> tensor<4x16x16xf32>
      %31 = linalg.generic {indexing_maps = [#map1, #map2, #map3], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%27, %28 : tensor<3x66x66xf32>, tensor<4x3x4x4xf32>) outs(%30 : tensor<4x16x16xf32>) {
      ^bb0(%in: f32, %in_5: f32, %out: f32):
        %33 = arith.mulf %in, %in_5 : f32
        %34 = arith.addf %out, %33 : f32
        linalg.yield %34 : f32
      } -> tensor<4x16x16xf32>
      %32 = linalg.generic {indexing_maps = [#map4, #map5, #map4], iterator_types = ["parallel", "parallel", "parallel"]} ins(%31, %cst_4 : tensor<4x16x16xf32>, tensor<4xf32>) outs(%29 : tensor<4x16x16xf32>) {
      ^bb0(%in: f32, %in_5: f32, %out: f32):
        %33 = arith.addf %in, %in_5 : f32
        %34 = arith.cmpf ugt, %33, %cst_3 : f32
        %35 = arith.select %34, %33, %cst_3 : f32
        linalg.yield %35 : f32
      } -> tensor<4x16x16xf32>
      iree_tensor_ext.dispatch.tensor.store %32, %arg8, offsets = [0, 0, 0], sizes = [4, 16, 16], strides = [1, 1, 1] : tensor<4x16x16xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x16x16xf32>>
      flow.return
    } count() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      flow.return %x, %y, %z : index, index, index
    }
    %17 = flow.dispatch.workgroups(%16) : (tensor<4x16x16xf32>) -> tensor<4x8x8xf32> =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x16x16xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x8x8xf32>>) {
      %cst_3 = arith.constant 0xFF800000 : f32
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0, 0, 0], sizes = [4, 16, 16], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4x16x16xf32>> -> tensor<4x16x16xf32>
      %28 = tensor.empty() : tensor<4x8x8xf32>
      %29 = tensor.empty() : tensor<2x2xf32>
      %30 = linalg.fill ins(%cst_3 : f32) outs(%28 : tensor<4x8x8xf32>) -> tensor<4x8x8xf32>
      %31 = linalg.generic {indexing_maps = [#map6, #map7, #map8], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction"]} ins(%27, %29 : tensor<4x16x16xf32>, tensor<2x2xf32>) outs(%30 : tensor<4x8x8xf32>) {
      ^bb0(%in: f32, %in_4: f32, %out: f32):
        %32 = arith.maximumf %out, %in : f32
        linalg.yield %32 : f32
      } -> tensor<4x8x8xf32>
      iree_tensor_ext.dispatch.tensor.store %31, %arg7, offsets = [0, 0, 0], sizes = [4, 8, 8], strides = [1, 1, 1] : tensor<4x8x8xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<4x8x8xf32>>
      flow.return
    } count() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      flow.return %x, %y, %z : index, index, index
    }
    %18 = flow.tensor.reshape %17 : tensor<4x8x8xf32> -> tensor<1x256x1x1xf32>
    %19 = flow.dispatch.workgroups(%18, %cst_2) : (tensor<1x256x1x1xf32>, tensor<1x256x8x1xf32>) -> tensor<1x1x1x8xf32> =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x256x1x1xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x256x8x1xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x1x1x8xf32>>) {
      %cst_3 = arith.constant 0.000000e+00 : f32
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0, 0, 0, 0], sizes = [1, 256, 1, 1], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x256x1x1xf32>> -> tensor<1x256x1x1xf32>
      %28 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0, 0, 0], sizes = [1, 256, 8, 1], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x256x8x1xf32>> -> tensor<1x256x8x1xf32>
      %29 = tensor.empty() : tensor<1x1x1x8xf32>
      %30 = linalg.fill ins(%cst_3 : f32) outs(%29 : tensor<1x1x1x8xf32>) -> tensor<1x1x1x8xf32>
      %31 = linalg.mmt4d ins(%27, %28 : tensor<1x256x1x1xf32>, tensor<1x256x8x1xf32>) outs(%30 : tensor<1x1x1x8xf32>) -> tensor<1x1x1x8xf32>
      iree_tensor_ext.dispatch.tensor.store %31, %arg8, offsets = [0, 0, 0, 0], sizes = [1, 1, 1, 8], strides = [1, 1, 1, 1] : tensor<1x1x1x8xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x1x1x8xf32>>
      flow.return
    } count() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      flow.return %x, %y, %z : index, index, index
    }
    %20 = flow.dispatch.workgroups(%19) : (tensor<1x1x1x8xf32>) -> tensor<1x2xf32> =
        (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x1x1x8xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2xf32>>) {
      %27 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0, 0, 0, 0], sizes = [1, 1, 1, 8], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x1x1x8xf32>> -> tensor<1x1x1x8xf32>
      %28 = tensor.empty() : tensor<1x2xf32>
      %unpack = linalg.unpack %27 outer_dims_perm = [0, 1] inner_dims_pos = [0, 1] inner_tiles = [1, 8] into %28 : tensor<1x1x1x8xf32> -> tensor<1x2xf32>
      iree_tensor_ext.dispatch.tensor.store %unpack, %arg7, offsets = [0, 0], sizes = [1, 2], strides = [1, 1] : tensor<1x2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2xf32>>
      flow.return
    } count() -> (index, index, index) {
      %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
      flow.return %x, %y, %z : index, index, index
    }
    %21 = scf.while (%arg6 = %2) : (tensor<2xf32>) -> tensor<2xf32> {
      %27 = flow.dispatch.workgroups(%arg6) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c0_3 = arith.constant 0 : index
        %c1_4 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_3] : tensor<2xf32>
          %extracted_5 = tensor.extract %76[%c1_4] : tensor<2xf32>
          %79 = arith.addf %extracted, %extracted_5 : f32
          %80 = math.cos %79 : f32
          linalg.yield %80 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %28 = flow.dispatch.workgroups(%arg6) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c0_3 = arith.constant 0 : index
        %c1_4 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_3] : tensor<2xf32>
          %extracted_5 = tensor.extract %76[%c1_4] : tensor<2xf32>
          %79 = arith.addf %extracted, %extracted_5 : f32
          %80 = math.cos %79 : f32
          %81 = math.cos %extracted : f32
          %82 = arith.addf %81, %80 : f32
          linalg.yield %82 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %29 = flow.dispatch.workgroups(%arg6) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c0_3 = arith.constant 0 : index
        %c1_4 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_3] : tensor<2xf32>
          %extracted_5 = tensor.extract %76[%c1_4] : tensor<2xf32>
          %79 = arith.addf %extracted, %extracted_5 : f32
          %80 = math.sin %79 : f32
          %81 = math.sin %extracted : f32
          %82 = arith.addf %81, %80 : f32
          linalg.yield %82 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %30 = flow.tensor.reshape %28 : tensor<f32> -> tensor<1xf32>
      %31 = flow.tensor.reshape %29 : tensor<f32> -> tensor<1xf32>
      %32 = tensor.empty() : tensor<2xf32>
      %33 = flow.tensor.update %30, %32[%c0] : tensor<1xf32> -> %32 as tensor<2xf32>
      %34 = flow.tensor.update %31, %33[%c1] : tensor<1xf32> -> %33 as tensor<2xf32>
      %35 = flow.tensor.reshape %34 : tensor<2xf32> -> tensor<1x2xf32>
      %36 = flow.dispatch.workgroups(%20, %35) : (tensor<1x2xf32>, tensor<1x2xf32>) -> tensor<1x2xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2xf32>>, %arg9: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2xf32>>) {
        %cst_3 = arith.constant dense<[[0.036251314, -0.0380887836]]> : tensor<1x2xf32>
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0], sizes = [1, 2], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2xf32>> -> tensor<1x2xf32>
        %77 = iree_tensor_ext.dispatch.tensor.load %arg8, offsets = [0, 0], sizes = [1, 2], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2xf32>> -> tensor<1x2xf32>
        %78 = tensor.empty() : tensor<1x2xf32>
        %79 = linalg.generic {indexing_maps = [#map9, #map9, #map9, #map9], iterator_types = ["parallel", "parallel"]} ins(%76, %cst_3, %77 : tensor<1x2xf32>, tensor<1x2xf32>, tensor<1x2xf32>) outs(%78 : tensor<1x2xf32>) {
        ^bb0(%in: f32, %in_4: f32, %in_5: f32, %out: f32):
          %80 = arith.addf %in, %in_4 : f32
          %81 = arith.subf %80, %in_5 : f32
          linalg.yield %81 : f32
        } -> tensor<1x2xf32>
        iree_tensor_ext.dispatch.tensor.store %79, %arg9, offsets = [0, 0], sizes = [1, 2], strides = [1, 1] : tensor<1x2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %37 = flow.tensor.reshape %36 : tensor<1x2xf32> -> tensor<1x2x1x1xf32>
      %38 = flow.dispatch.workgroups(%arg6) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %cst_3 = arith.constant -1.000000e+00 : f32
        %c0_4 = arith.constant 0 : index
        %c1_5 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_4] : tensor<2xf32>
          %extracted_6 = tensor.extract %76[%c1_5] : tensor<2xf32>
          %79 = arith.addf %extracted, %extracted_6 : f32
          %80 = math.sin %79 : f32
          %81 = math.sin %extracted : f32
          %82 = arith.mulf %81, %cst_3 : f32
          %83 = arith.subf %82, %80 : f32
          linalg.yield %83 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %39 = flow.dispatch.workgroups(%arg6) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %cst_3 = arith.constant -1.000000e+00 : f32
        %c0_4 = arith.constant 0 : index
        %c1_5 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_4] : tensor<2xf32>
          %extracted_6 = tensor.extract %76[%c1_5] : tensor<2xf32>
          %79 = arith.addf %extracted, %extracted_6 : f32
          %80 = math.sin %79 : f32
          %81 = arith.mulf %80, %cst_3 : f32
          linalg.yield %81 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %40 = flow.tensor.reshape %38 : tensor<f32> -> tensor<1xf32>
      %41 = flow.tensor.reshape %39 : tensor<f32> -> tensor<1xf32>
      %42 = flow.tensor.update %40, %32[%c0] : tensor<1xf32> -> %32 as tensor<2xf32>
      %43 = flow.tensor.update %41, %42[%c1] : tensor<1xf32> -> %42 as tensor<2xf32>
      %44 = flow.tensor.reshape %27 : tensor<f32> -> tensor<1xf32>
      %45 = flow.tensor.update %44, %33[%c1] : tensor<1xf32> -> %33 as tensor<2xf32>
      %46 = flow.tensor.reshape %43 : tensor<2xf32> -> tensor<1x2xf32>
      %47 = flow.tensor.reshape %45 : tensor<2xf32> -> tensor<1x2xf32>
      %48 = tensor.empty() : tensor<2x2xf32>
      %49 = flow.tensor.update %46, %48[%c0, %c0] : tensor<1x2xf32> -> %48 as tensor<2x2xf32>
      %50 = flow.tensor.update %47, %49[%c1, %c0] : tensor<1x2xf32> -> %49 as tensor<2x2xf32>
      %51 = flow.dispatch.workgroups(%50) : (tensor<2x2xf32>) -> tensor<2xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>) {
        %c0_3 = arith.constant 0 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0], sizes = [2, 2], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>> -> tensor<2x2xf32>
        %77 = tensor.empty() : tensor<2xf32>
        %78 = linalg.generic {indexing_maps = [#map10], iterator_types = ["parallel"]} outs(%77 : tensor<2xf32>) {
        ^bb0(%out: f32):
          %79 = linalg.index 0 : index
          %extracted = tensor.extract %76[%c0_3, %79] : tensor<2x2xf32>
          linalg.yield %extracted : f32
        } -> tensor<2xf32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [0], sizes = [2], strides = [1] : tensor<2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %52 = flow.dispatch.workgroups(%51) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c0_3 = arith.constant 0 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_3] : tensor<2xf32>
          linalg.yield %extracted : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %53 = flow.tensor.reshape %52 : tensor<f32> -> tensor<1xf32>
      %54 = flow.dispatch.workgroups(%50) : (tensor<2x2xf32>) -> tensor<2xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>) {
        %c1_3 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0], sizes = [2, 2], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>> -> tensor<2x2xf32>
        %77 = tensor.empty() : tensor<2xf32>
        %78 = linalg.generic {indexing_maps = [#map10], iterator_types = ["parallel"]} outs(%77 : tensor<2xf32>) {
        ^bb0(%out: f32):
          %79 = linalg.index 0 : index
          %extracted = tensor.extract %76[%c1_3, %79] : tensor<2x2xf32>
          linalg.yield %extracted : f32
        } -> tensor<2xf32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [0], sizes = [2], strides = [1] : tensor<2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %55 = flow.dispatch.workgroups(%54) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c1_3] : tensor<2xf32>
          linalg.yield %extracted : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %56 = flow.tensor.reshape %55 : tensor<f32> -> tensor<1xf32>
      %57 = flow.dispatch.workgroups(%51) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c1_3] : tensor<2xf32>
          %79 = arith.negf %extracted : f32
          linalg.yield %79 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %58 = flow.tensor.reshape %57 : tensor<f32> -> tensor<1xf32>
      %59 = flow.tensor.update %56, %32[%c0] : tensor<1xf32> -> %32 as tensor<2xf32>
      %60 = flow.tensor.update %58, %59[%c1] : tensor<1xf32> -> %59 as tensor<2xf32>
      %61 = flow.dispatch.workgroups(%54) : (tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c0_3 = arith.constant 0 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = tensor.empty() : tensor<f32>
        %78 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%77 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted = tensor.extract %76[%c0_3] : tensor<2xf32>
          %79 = arith.negf %extracted : f32
          linalg.yield %79 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %78, %arg8, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %62 = flow.tensor.reshape %61 : tensor<f32> -> tensor<1xf32>
      %63 = flow.tensor.update %62, %32[%c0] : tensor<1xf32> -> %32 as tensor<2xf32>
      %64 = flow.tensor.update %53, %63[%c1] : tensor<1xf32> -> %63 as tensor<2xf32>
      %65 = flow.tensor.reshape %60 : tensor<2xf32> -> tensor<1x2xf32>
      %66 = flow.tensor.reshape %64 : tensor<2xf32> -> tensor<1x2xf32>
      %67 = flow.tensor.update %65, %48[%c0, %c0] : tensor<1x2xf32> -> %48 as tensor<2x2xf32>
      %68 = flow.tensor.update %66, %67[%c1, %c0] : tensor<1x2xf32> -> %67 as tensor<2x2xf32>
      %69 = flow.dispatch.workgroups(%51, %54, %68) : (tensor<2xf32>, tensor<2xf32>, tensor<2x2xf32>) -> tensor<1x2x8x1xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg9: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>>, %arg10: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2x8x1xf32>>) {
        %cst_3 = arith.constant 9.99999997E-7 : f32
        %cst_4 = arith.constant 0.000000e+00 : f32
        %cst_5 = arith.constant 1.000000e+00 : f32
        %c0_6 = arith.constant 0 : index
        %c1_7 = arith.constant 1 : index
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = iree_tensor_ext.dispatch.tensor.load %arg8, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %78 = iree_tensor_ext.dispatch.tensor.load %arg9, offsets = [0, 0], sizes = [2, 2], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x2xf32>> -> tensor<2x2xf32>
        %79 = tensor.empty() : tensor<1x2x8x1xf32>
        %80 = tensor.empty() : tensor<2x2xf32>
        %81 = linalg.generic {indexing_maps = [#map9, #map9], iterator_types = ["parallel", "parallel"]} ins(%78 : tensor<2x2xf32>) outs(%80 : tensor<2x2xf32>) {
        ^bb0(%in: f32, %out: f32):
          %extracted = tensor.extract %76[%c0_6] : tensor<2xf32>
          %extracted_8 = tensor.extract %77[%c1_7] : tensor<2xf32>
          %extracted_9 = tensor.extract %76[%c1_7] : tensor<2xf32>
          %extracted_10 = tensor.extract %77[%c0_6] : tensor<2xf32>
          %82 = arith.mulf %extracted_9, %extracted_10 : f32
          %83 = arith.mulf %extracted, %extracted_8 : f32
          %84 = arith.subf %83, %82 : f32
          %85 = arith.addf %84, %cst_3 : f32
          %86 = arith.cmpf one, %85, %cst_4 : f32
          cf.assert %86, "unimplemented: tensor with zero element"
          %87 = arith.divf %cst_5, %85 : f32
          %88 = arith.mulf %in, %87 : f32
          linalg.yield %88 : f32
        } -> tensor<2x2xf32>
        %pack = linalg.pack %81 padding_value(%cst_4 : f32) outer_dims_perm = [0, 1] inner_dims_pos = [0, 1] inner_tiles = [8, 1] into %79 : tensor<2x2xf32> -> tensor<1x2x8x1xf32>
        iree_tensor_ext.dispatch.tensor.store %pack, %arg10, offsets = [0, 0, 0, 0], sizes = [1, 2, 8, 1], strides = [1, 1, 1, 1] : tensor<1x2x8x1xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x2x8x1xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %70 = flow.dispatch.workgroups(%37, %69) : (tensor<1x2x1x1xf32>, tensor<1x2x8x1xf32>) -> tensor<1x1x1x8xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2x1x1xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2x8x1xf32>>, %arg9: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x1x1x8xf32>>) {
        %cst_3 = arith.constant 0.000000e+00 : f32
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0, 0, 0], sizes = [1, 2, 1, 1], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2x1x1xf32>> -> tensor<1x2x1x1xf32>
        %77 = iree_tensor_ext.dispatch.tensor.load %arg8, offsets = [0, 0, 0, 0], sizes = [1, 2, 8, 1], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x2x8x1xf32>> -> tensor<1x2x8x1xf32>
        %78 = tensor.empty() : tensor<1x1x1x8xf32>
        %79 = linalg.fill ins(%cst_3 : f32) outs(%78 : tensor<1x1x1x8xf32>) -> tensor<1x1x1x8xf32>
        %80 = linalg.mmt4d ins(%76, %77 : tensor<1x2x1x1xf32>, tensor<1x2x8x1xf32>) outs(%79 : tensor<1x1x1x8xf32>) -> tensor<1x1x1x8xf32>
        iree_tensor_ext.dispatch.tensor.store %80, %arg9, offsets = [0, 0, 0, 0], sizes = [1, 1, 1, 8], strides = [1, 1, 1, 1] : tensor<1x1x1x8xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<1x1x1x8xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %71 = flow.tensor.reshape %70 : tensor<1x1x1x8xf32> -> tensor<1x8xf32>
      %72 = flow.dispatch.workgroups(%71, %arg6) : (tensor<1x8xf32>, tensor<2xf32>) -> tensor<2xf32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x8xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg9: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>) {
        %cst_3 = arith.constant 1.000000e-01 : f32
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0, 0], sizes = [1, 8], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x8xf32>> -> tensor<1x8xf32>
        %77 = iree_tensor_ext.dispatch.tensor.load %arg8, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %78 = tensor.empty() : tensor<2xf32>
        %unpack = linalg.unpack %76 outer_dims_perm = [0] inner_dims_pos = [0] inner_tiles = [8] into %78 : tensor<1x8xf32> -> tensor<2xf32>
        %79 = linalg.generic {indexing_maps = [#map10, #map10, #map10], iterator_types = ["parallel"]} ins(%77, %unpack : tensor<2xf32>, tensor<2xf32>) outs(%78 : tensor<2xf32>) {
        ^bb0(%in: f32, %in_4: f32, %out: f32):
          %80 = arith.mulf %in_4, %cst_3 : f32
          %81 = arith.addf %in, %80 : f32
          linalg.yield %81 : f32
        } -> tensor<2xf32>
        iree_tensor_ext.dispatch.tensor.store %79, %arg9, offsets = [0], sizes = [2], strides = [1] : tensor<2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %73 = flow.dispatch.workgroups(%72, %arg6) : (tensor<2xf32>, tensor<2xf32>) -> tensor<f32> =
          (%arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg9: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %cst_3 = arith.constant 0.000000e+00 : f32
        %76 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %77 = iree_tensor_ext.dispatch.tensor.load %arg8, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %78 = tensor.empty() : tensor<f32>
        %79 = linalg.fill ins(%cst_3 : f32) outs(%78 : tensor<f32>) -> tensor<f32>
        %80 = linalg.generic {indexing_maps = [#map10, #map10, #map11], iterator_types = ["reduction"]} ins(%76, %77 : tensor<2xf32>, tensor<2xf32>) outs(%79 : tensor<f32>) {
        ^bb0(%in: f32, %in_4: f32, %out: f32):
          %81 = arith.subf %in, %in_4 : f32
          %82 = arith.mulf %81, %81 : f32
          %83 = arith.addf %82, %out : f32
          linalg.yield %83 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %80, %arg9, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %74 = flow.tensor.load %73 : tensor<f32>
      %75 = arith.cmpf ogt, %74, %cst : f32
      scf.condition(%75) %72 : tensor<2xf32>
    } do {
    ^bb0(%arg6: tensor<2xf32>):
      scf.yield %arg6 : tensor<2xf32>
    }
    %22 = flow.tensor.load %21[%c1] : tensor<2xf32>
    %23 = arith.cmpf ogt, %22, %cst_0 : f32
    %24 = scf.if %23 -> (tensor<19x2xf32>) {
      %27 = flow.dispatch.workgroups(%21) : (tensor<2xf32>) -> tensor<f32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %c0_4 = arith.constant 0 : index
        %36 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %37 = tensor.empty() : tensor<f32>
        %extracted = tensor.extract %36[%c1_3] : tensor<2xf32>
        %38 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%37 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted_5 = tensor.extract %36[%c0_4] : tensor<2xf32>
          %39 = arith.addf %extracted_5, %extracted : f32
          %40 = math.cos %39 : f32
          %41 = math.cos %extracted_5 : f32
          %42 = arith.addf %41, %40 : f32
          linalg.yield %42 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %38, %arg7, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %28 = flow.dispatch.workgroups(%21) : (tensor<2xf32>) -> tensor<f32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %c0_4 = arith.constant 0 : index
        %36 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %37 = tensor.empty() : tensor<f32>
        %extracted = tensor.extract %36[%c1_3] : tensor<2xf32>
        %38 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%37 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted_5 = tensor.extract %36[%c0_4] : tensor<2xf32>
          %39 = arith.addf %extracted_5, %extracted : f32
          %40 = math.sin %39 : f32
          %41 = math.sin %extracted_5 : f32
          %42 = arith.addf %41, %40 : f32
          linalg.yield %42 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %38, %arg7, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %29 = flow.tensor.reshape %27 : tensor<f32> -> tensor<1xf32>
      %30 = flow.tensor.reshape %28 : tensor<f32> -> tensor<1xf32>
      %31 = tensor.empty() : tensor<2xf32>
      %32 = flow.tensor.update %29, %31[%c0] : tensor<1xf32> -> %31 as tensor<2xf32>
      %33 = flow.tensor.update %30, %32[%c1] : tensor<1xf32> -> %32 as tensor<2xf32>
      %34 = flow.tensor.splat %cst_0 : tensor<19x2xf32>
      %35 = flow.dispatch.workgroups(%33, %34) : (tensor<2xf32>, tensor<19x2xf32>) -> %34 =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readwrite:tensor<19x2xf32>>) {
        %cst_3 = arith.constant dense<[1.000000e+00, 0.888888895, 0.777777791, 0.666666627, 0.555555582, 0.444444418, 0.333333313, 0.222222209, 0.111111104, 0.000000e+00]> : tensor<10xf32>
        %cst_4 = arith.constant dense<[0.000000e+00, -1.000000e-01]> : tensor<2xf32>
        %cst_5 = arith.constant dense<[0.000000e+00, 0.111111112, 0.222222224, 0.333333343, 0.444444448, 0.555555582, 0.666666686, 0.777777791, 0.888888895, 1.000000e+00]> : tensor<10xf32>
        %36 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %37 = tensor.empty() : tensor<10x2xf32>
        %38 = linalg.generic {indexing_maps = [#map12, #map13, #map12, #map13, #map9], iterator_types = ["parallel", "parallel"]} ins(%36, %cst_3, %cst_4, %cst_5 : tensor<2xf32>, tensor<10xf32>, tensor<2xf32>, tensor<10xf32>) outs(%37 : tensor<10x2xf32>) {
        ^bb0(%in: f32, %in_6: f32, %in_7: f32, %in_8: f32, %out: f32):
          %39 = arith.addf %in, %in_7 : f32
          %40 = arith.mulf %39, %in_8 : f32
          %41 = arith.mulf %in, %in_6 : f32
          %42 = arith.addf %41, %40 : f32
          linalg.yield %42 : f32
        } -> tensor<10x2xf32>
        iree_tensor_ext.dispatch.tensor.store %38, %arg7, offsets = [0, 0], sizes = [10, 2], strides = [1, 1] : tensor<10x2xf32> -> !iree_tensor_ext.dispatch.tensor<readwrite:tensor<19x2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      scf.yield %35 : tensor<19x2xf32>
    } else {
      %27 = flow.dispatch.workgroups(%21) : (tensor<2xf32>) -> tensor<f32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %c0_4 = arith.constant 0 : index
        %40 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %41 = tensor.empty() : tensor<f32>
        %extracted = tensor.extract %40[%c1_3] : tensor<2xf32>
        %42 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%41 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted_5 = tensor.extract %40[%c0_4] : tensor<2xf32>
          %43 = arith.addf %extracted_5, %extracted : f32
          %44 = math.cos %43 : f32
          %45 = math.cos %extracted_5 : f32
          %46 = arith.addf %45, %44 : f32
          linalg.yield %46 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %42, %arg7, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %28 = flow.dispatch.workgroups(%21) : (tensor<2xf32>) -> tensor<f32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %c1_3 = arith.constant 1 : index
        %c0_4 = arith.constant 0 : index
        %40 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %41 = tensor.empty() : tensor<f32>
        %extracted = tensor.extract %40[%c1_3] : tensor<2xf32>
        %42 = linalg.generic {indexing_maps = [#map], iterator_types = []} outs(%41 : tensor<f32>) {
        ^bb0(%out: f32):
          %extracted_5 = tensor.extract %40[%c0_4] : tensor<2xf32>
          %43 = arith.addf %extracted_5, %extracted : f32
          %44 = math.sin %43 : f32
          %45 = math.sin %extracted_5 : f32
          %46 = arith.addf %45, %44 : f32
          linalg.yield %46 : f32
        } -> tensor<f32>
        iree_tensor_ext.dispatch.tensor.store %42, %arg7, offsets = [], sizes = [], strides = [] : tensor<f32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %29 = flow.tensor.reshape %27 : tensor<f32> -> tensor<1xf32>
      %30 = flow.tensor.reshape %28 : tensor<f32> -> tensor<1xf32>
      %31 = tensor.empty() : tensor<2xf32>
      %32 = flow.tensor.update %29, %31[%c0] : tensor<1xf32> -> %31 as tensor<2xf32>
      %33 = flow.tensor.update %30, %32[%c1] : tensor<1xf32> -> %32 as tensor<2xf32>
      %34 = flow.dispatch.workgroups(%33) : (tensor<2xf32>) -> tensor<2xf32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>) {
        %cst_3 = arith.constant dense<[-2.000000e-01, 0.000000e+00]> : tensor<2xf32>
        %40 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %41 = tensor.empty() : tensor<2xf32>
        %42 = linalg.generic {indexing_maps = [#map10, #map10, #map10], iterator_types = ["parallel"]} ins(%40, %cst_3 : tensor<2xf32>, tensor<2xf32>) outs(%41 : tensor<2xf32>) {
        ^bb0(%in: f32, %in_4: f32, %out: f32):
          %43 = arith.addf %in, %in_4 : f32
          linalg.yield %43 : f32
        } -> tensor<2xf32>
        iree_tensor_ext.dispatch.tensor.store %42, %arg7, offsets = [0], sizes = [2], strides = [1] : tensor<2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %35 = flow.dispatch.workgroups(%33, %34) : (tensor<2xf32>, tensor<2xf32>) -> tensor<10x2xf32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg8: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<10x2xf32>>) {
        %cst_3 = arith.constant dense<[1.000000e+00, 0.888888895, 0.777777791, 0.666666627, 0.555555582, 0.444444418, 0.333333313, 0.222222209, 0.111111104, 0.000000e+00]> : tensor<10xf32>
        %cst_4 = arith.constant dense<[0.000000e+00, 0.111111112, 0.222222224, 0.333333343, 0.444444448, 0.555555582, 0.666666686, 0.777777791, 0.888888895, 1.000000e+00]> : tensor<10xf32>
        %40 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %41 = iree_tensor_ext.dispatch.tensor.load %arg7, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %42 = tensor.empty() : tensor<10x2xf32>
        %43 = linalg.generic {indexing_maps = [#map12, #map13, #map12, #map13, #map9], iterator_types = ["parallel", "parallel"]} ins(%40, %cst_3, %41, %cst_4 : tensor<2xf32>, tensor<10xf32>, tensor<2xf32>, tensor<10xf32>) outs(%42 : tensor<10x2xf32>) {
        ^bb0(%in: f32, %in_5: f32, %in_6: f32, %in_7: f32, %out: f32):
          %44 = arith.mulf %in_6, %in_7 : f32
          %45 = arith.mulf %in, %in_5 : f32
          %46 = arith.addf %45, %44 : f32
          linalg.yield %46 : f32
        } -> tensor<10x2xf32>
        iree_tensor_ext.dispatch.tensor.store %43, %arg8, offsets = [0, 0], sizes = [10, 2], strides = [1, 1] : tensor<10x2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<10x2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %36 = flow.dispatch.workgroups(%34) : (tensor<2xf32>) -> tensor<9x2xf32> =
          (%arg6: !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>>, %arg7: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<9x2xf32>>) {
        %cst_3 = arith.constant dense<[1.000000e+00, 0.888888895, 0.777777791, 0.666666627, 0.555555582, 0.444444418, 0.333333313, 0.222222209, 0.111111104, 0.000000e+00]> : tensor<10xf32>
        %cst_4 = arith.constant dense<[0.000000e+00, -1.000000e-01]> : tensor<2xf32>
        %cst_5 = arith.constant dense<[0.000000e+00, 0.111111112, 0.222222224, 0.333333343, 0.444444448, 0.555555582, 0.666666686, 0.777777791, 0.888888895, 1.000000e+00]> : tensor<10xf32>
        %40 = iree_tensor_ext.dispatch.tensor.load %arg6, offsets = [0], sizes = [2], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2xf32>> -> tensor<2xf32>
        %41 = tensor.empty() : tensor<9x2xf32>
        %extracted_slice = tensor.extract_slice %cst_3[1] [9] [1] : tensor<10xf32> to tensor<9xf32>
        %extracted_slice_6 = tensor.extract_slice %cst_5[1] [9] [1] : tensor<10xf32> to tensor<9xf32>
        %42 = linalg.generic {indexing_maps = [#map12, #map13, #map12, #map13, #map9], iterator_types = ["parallel", "parallel"]} ins(%40, %extracted_slice, %cst_4, %extracted_slice_6 : tensor<2xf32>, tensor<9xf32>, tensor<2xf32>, tensor<9xf32>) outs(%41 : tensor<9x2xf32>) {
        ^bb0(%in: f32, %in_7: f32, %in_8: f32, %in_9: f32, %out: f32):
          %43 = arith.addf %in, %in_8 : f32
          %44 = arith.mulf %43, %in_9 : f32
          %45 = arith.mulf %in, %in_7 : f32
          %46 = arith.addf %45, %44 : f32
          linalg.yield %46 : f32
        } -> tensor<9x2xf32>
        iree_tensor_ext.dispatch.tensor.store %42, %arg7, offsets = [0, 0], sizes = [9, 2], strides = [1, 1] : tensor<9x2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<9x2xf32>>
        flow.return
      } count() -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        flow.return %x, %y, %z : index, index, index
      }
      %37 = tensor.empty() : tensor<19x2xf32>
      %38 = flow.tensor.update %35, %37[%c0, %c0] : tensor<10x2xf32> -> %37 as tensor<19x2xf32>
      %39 = flow.tensor.update %36, %38[%c10, %c0] : tensor<9x2xf32> -> %38 as tensor<19x2xf32>
      scf.yield %39 : tensor<19x2xf32>
    }
    %25 = hal.tensor.export %24 : tensor<19x2xf32> -> !hal.buffer_view
    %26 = hal.tensor.export %11 : tensor<3xf32> -> !hal.buffer_view
    hal.fence.signal<%arg5 : !hal.fence>
    util.return %25, %26 : !hal.buffer_view, !hal.buffer_view
  }
  util.func public @main(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view, %arg3: !hal.buffer_view) -> (!hal.buffer_view, !hal.buffer_view) attributes {iree.abi.stub} {
    %c-1_i32 = arith.constant -1 : i32
    %0 = util.null : !hal.fence
    %c0 = arith.constant 0 : index
    %device_0 = hal.devices.get %c0 : !hal.device
    %fence = hal.fence.create device(%device_0 : !hal.device) flags("None") : !hal.fence
    %1:2 = util.call @_main$async(%arg0, %arg1, %arg2, %arg3, %0, %fence) : (!hal.buffer_view, !hal.buffer_view, !hal.buffer_view, !hal.buffer_view, !hal.fence, !hal.fence) -> (!hal.buffer_view, !hal.buffer_view)
    %status = hal.fence.await until([%fence]) timeout_millis(%c-1_i32) flags("None") : i32
    util.return %1#0, %1#1 : !hal.buffer_view, !hal.buffer_view
  }
}
