...
#map_scalar_in_2d = affine_map<() -> ()>
#map_scalar_in_1d = affine_map<() -> ()>
...
module attributes {transform.with_named_sequence} {
      ...
      builtin.module {
        ...
        func.func @qgemm_i8_bias_i32() {
          ...
          %A_mem = hal.interface.binding.subspan layout(#pipeline_layout) binding(0)
            alignment(64) offset(%c0) : memref<1x?xi8>{%M, %K}
          ...
          %C_out_mem = hal.interface.binding.subspan layout(#pipeline_layout) binding(3)
            alignment(64) offset(%c0) : memref<1x?xi32>{%M, %N}
  ...
  util.func private @call_qgemm_dequant(
    ...
    %C_i32_empty = tensor.empty(%M_idx, %N_idx) : tensor<1x?xi32>
    ...
    ins(%C_i32_result : tensor<1x?xi32>{%N_idx}, %Scale_f32 : f32)
    outs(%C_f32_empty : tensor<1x?xf32>{%N_idx}) {
    ^bb0(%in_i32: i32, %in_scale: f32, %out_f32: f32):
      %in_f32 = arith.sitofp %in_i32 : i32 to f32
      %scaled = arith.mulf %in_f32, %in_scale : f32
      linalg.yield %scaled : f32
    } -> tensor<1x?xf32>{%N_idx}

    util.return %C_f32_result : tensor<1x?xf32>{%N_idx}
  }
  ...
  transform.named_sequence @match_qgemm_bias_fused_scale(
    ...
    ^bb0(%A: tensor<1x?xi8>, %B: tensor<?x?xi8>, %Bias_i32: tensor<?xi32>, 
      ...
      %empty_i32 = tensor.empty(%dim_0, %dim_1) {"match.dynamic_dim_only"} : tensor<1x?xi32>
      ...
      // Get the scalar f32 scale for Matmul
      %Scale_Matmul = tensor.extract %Scale_Matmul_tensor[] : tensor<f32>

      // Get the scalar f32 scale for Bias
      %Scale_Bias = tensor.extract %Scale_Bias_tensor[] : tensor<f32>

      // 2. Dequant Matmul
      %empty_f32_C = tensor.empty(%dim_0, %dim_1) {"match.dynamic_dim_only"} : tensor<1x?xf32>
      %C_f32 = linalg.generic {
        indexing_maps = [#map_2d, #map_scalar_in_2d, #map_2d],
        iterator_types = ["parallel", "parallel"]
      } 
      ins(%C_i32 : tensor<1x?xi32>, %Scale_Matmul : f32)
      ...
      ins(%Bias_i32 : tensor<?xi32>, %Scale_Bias : f32)
      ...
      // 4. Add Bias 
      %empty_f32_Out = tensor.empty(%dim_0, %dim_1) {"match.dynamic_dim_only"} : tensor<1x?xf32>
      %Result_f32 = linalg.generic {
        indexing_maps = [#map_2d, #map_1d, #map_2d],
        iterator_types = ["parallel", "parallel"]
