module {
  ...
  hal.executable private @main_graph$async_dispatch_11 {
    hal.executable.variant public @system_elf_x86_64 target(<"llvm-cpu", "system-elf-x86_64", {cpu = "znver3", cpu_features = "+prfchw,-cldemote,+avx,+aes,+sahf,+pclmul,-xop,+crc32,-amx-fp8,+xsaves,-avx512fp16,-usermsr,-sm4,-egpr,+sse4.1,-avx10.1,-avx512ifma,+xsave,+sse4.2,-tsxldtrk,-sm3,-ptwrite,-widekl,-movrs,+invpcid,+64bit,+xsavec,-avx512vpopcntdq,+cmov,-avx512vp2intersect,-avx512cd,+movbe,-avxvnniint8,-ccmp,-amx-int8,-kl,-sha512,-avxvnni,-rtm,+adx,+avx2,-hreset,-movdiri,-serialize,+vpclmulqdq,-avx512vl,-uintr,-cf,+clflushopt,-raoint,-cmpccxadd,+bmi,-amx-tile,+sse,-gfni,-avxvnniint16,-amx-fp16,-zu,-ndd,+xsaveopt,+rdrnd,-avx512f,-amx-bf16,-avx512bf16,-avx512vnni,-push2pop2,+cx8,-avx512bw,+sse3,+pku,-nf,-amx-tf32,-amx-avx512,+fsgsbase,+clzero,+mwaitx,-lwp,+lzcnt,+sha,-movdir64b,-ppx,+wbnoinvd,-enqcmd,-amx-transpose,-avxneconvert,-tbm,-pconfig,-amx-complex,+ssse3,+cx16,-avx10.2,+bmi2,+fma,+popcnt,-avxifma,+f16c,-avx512bitalg,+rdpru,+clwb,+mmx,+sse2,+rdseed,-avx512vbmi2,-prefetchi,-amx-movrs,+rdpid,-fma4,-avx512vbmi,+shstk,+vaes,-waitpkg,-sgx,+fxsr,-avx512dq,+sse4a", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, link_embedded = false, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_triple = "x86_64-unknown-linux-gnu"}>) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert>} {
          ...
          %12 = linalg.fill {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 8]>} ins(%cst : f32) outs(%11 : tensor<128x7x7xf32>) -> tensor<128x7x7xf32>
          %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4, d5) -> (d3, d1 + d4, d2 + d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d3, d4, d5)>, affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%6, %7 : tensor<128x9x9xf32>, tensor<128x128x3x3xf32>) outs(%12 : tensor<128x7x7xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<distribution = [16, 1, 7, 0, 0, 0], vector_common_parallel = [1, 1, 8, 0, 0, 0], vector_reduction = [0, 0, 0, 1, 1, 8]>} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %15 = arith.mulf %in, %in_0 : f32
            %16 = arith.addf %out, %15 : f32
            linalg.yield %16 : f32
          } -> tensor<128x7x7xf32>
          %14 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%8, %9, %13, %10 : tensor<128x7x7xf32>, tensor<128xf32>, tensor<128x7x7xf32>, tensor<128xf32>) outs(%11 : tensor<128x7x7xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 8]>} {
  ...
  util.global private mutable @main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer
  util.initializer {
    ...
    util.global.store %buffer, @main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer = util.global.load @main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    ...
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@main_graph$async_dispatch_11::@system_elf_x86_64::@main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@main_graph$async_dispatch_11) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@main_graph$async_dispatch_11::@system_elf_x86_64::@main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer)[%c0, %c2616640], 
        (%main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer)[%c2616832, %c1279616], 
        (%main_graph$async_dispatch_11_system_elf_x86_64_main_graph$async_dispatch_11_conv_128x7x7x128x3x3_f32_buffer : !hal.buffer)[%c3896576, %c2616640]
      ]) flags("None")
