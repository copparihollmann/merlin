module {
  ...
  hal.executable private @main_graph$async_dispatch_45 {
    hal.executable.variant public @system_elf_x86_64 target(<"llvm-cpu", "system-elf-x86_64", {cpu = "znver3", cpu_features = "+prfchw,-cldemote,+avx,+aes,+sahf,+pclmul,-xop,+crc32,-amx-fp8,+xsaves,-avx512fp16,-usermsr,-sm4,-egpr,+sse4.1,-avx10.1,-avx512ifma,+xsave,+sse4.2,-tsxldtrk,-sm3,-ptwrite,-widekl,-movrs,+invpcid,+64bit,+xsavec,-avx512vpopcntdq,+cmov,-avx512vp2intersect,-avx512cd,+movbe,-avxvnniint8,-ccmp,-amx-int8,-kl,-sha512,-avxvnni,-rtm,+adx,+avx2,-hreset,-movdiri,-serialize,+vpclmulqdq,-avx512vl,-uintr,-cf,+clflushopt,-raoint,-cmpccxadd,+bmi,-amx-tile,+sse,-gfni,-avxvnniint16,-amx-fp16,-zu,-ndd,+xsaveopt,+rdrnd,-avx512f,-amx-bf16,-avx512bf16,-avx512vnni,-push2pop2,+cx8,-avx512bw,+sse3,+pku,-nf,-amx-tf32,-amx-avx512,+fsgsbase,+clzero,+mwaitx,-lwp,+lzcnt,+sha,-movdir64b,-ppx,+wbnoinvd,-enqcmd,-amx-transpose,-avxneconvert,-tbm,-pconfig,-amx-complex,+ssse3,+cx16,-avx10.2,+bmi2,+fma,+popcnt,-avxifma,+f16c,-avx512bitalg,+rdpru,+clwb,+mmx,+sse2,+rdseed,-avx512vbmi2,-prefetchi,-amx-movrs,+rdpid,-fma4,-avx512vbmi,+shstk,+vaes,-waitpkg,-sgx,+fxsr,-avx512dq,+sse4a", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, link_embedded = false, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_triple = "x86_64-unknown-linux-gnu"}>) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_45_matvec_like_50176x32_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %4 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64], vector_inner_parallel = [1]>} ins(%cst_1 : f32) outs(%3 : tensor<50176xf32>) -> tensor<50176xf32>
          %5 = tensor.empty() : tensor<32x50176xf32>
          %6 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} outs(%5 : tensor<32x50176xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64, 0], vector_inner_parallel = [1, 8]>} {
          ^bb0(%out: f32):
          ...
          %7 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%6, %cst_0 : tensor<32x50176xf32>, tensor<32xf32>) outs(%4 : tensor<50176xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64, 0], distribution = [64, 0], vector_reduction = [0, 8]>} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
            %9 = arith.mulf %in, %in_2 : f32
            %10 = arith.addf %out, %9 : f32
            linalg.yield %10 : f32
          } -> tensor<50176xf32>
          %8 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%7 : tensor<50176xf32>) outs(%3 : tensor<50176xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64], vector_inner_parallel = [1]>} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer : !hal.buffer
  util.initializer {
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %allocator = hal.device.allocator<%device : !hal.device> : !hal.allocator
    %memory_type = hal.memory_type<"DeviceVisible|DeviceLocal"> : i32
    %buffer_usage = hal.buffer_usage<"TransferSource|TransferTarget|Transfer|DispatchStorageRead|DispatchStorageWrite|DispatchStorage"> : i32
    %c10738944 = arith.constant 10738944 : index
    %buffer = hal.allocator.allocate<%allocator : !hal.allocator> affinity(%queue_affinity) type(%memory_type) usage(%buffer_usage) : !hal.buffer{%c10738944}
    util.global.store %buffer, @main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer = util.global.load @main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
    %c10538048 = arith.constant 10538048 : index
    %c10538240 = arith.constant 10538240 : index
    %c200704 = arith.constant 200704 : index
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@main_graph$async_dispatch_45::@system_elf_x86_64::@main_graph$async_dispatch_45_matvec_like_50176x32_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@main_graph$async_dispatch_45) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@main_graph$async_dispatch_45::@system_elf_x86_64::@main_graph$async_dispatch_45_matvec_like_50176x32_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) bindings([
        (%main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer : !hal.buffer)[%c0, %c10538048], 
        (%main_graph$async_dispatch_45_system_elf_x86_64_main_graph$async_dispatch_45_matvec_like_50176x32_f32_buffer : !hal.buffer)[%c10538240, %c200704]
      ]) flags("None")
