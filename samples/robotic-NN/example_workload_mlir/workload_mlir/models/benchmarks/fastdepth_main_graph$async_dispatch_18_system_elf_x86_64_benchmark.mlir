module {
  ...
  hal.executable private @main_graph$async_dispatch_18 {
    hal.executable.variant public @system_elf_x86_64 target(<"llvm-cpu", "system-elf-x86_64", {cpu = "znver3", cpu_features = "+prfchw,-cldemote,+avx,+aes,+sahf,+pclmul,-xop,+crc32,-amx-fp8,+xsaves,-avx512fp16,-usermsr,-sm4,-egpr,+sse4.1,-avx10.1,-avx512ifma,+xsave,+sse4.2,-tsxldtrk,-sm3,-ptwrite,-widekl,-movrs,+invpcid,+64bit,+xsavec,-avx512vpopcntdq,+cmov,-avx512vp2intersect,-avx512cd,+movbe,-avxvnniint8,-ccmp,-amx-int8,-kl,-sha512,-avxvnni,-rtm,+adx,+avx2,-hreset,-movdiri,-serialize,+vpclmulqdq,-avx512vl,-uintr,-cf,+clflushopt,-raoint,-cmpccxadd,+bmi,-amx-tile,+sse,-gfni,-avxvnniint16,-amx-fp16,-zu,-ndd,+xsaveopt,+rdrnd,-avx512f,-amx-bf16,-avx512bf16,-avx512vnni,-push2pop2,+cx8,-avx512bw,+sse3,+pku,-nf,-amx-tf32,-amx-avx512,+fsgsbase,+clzero,+mwaitx,-lwp,+lzcnt,+sha,-movdir64b,-ppx,+wbnoinvd,-enqcmd,-amx-transpose,-avxneconvert,-tbm,-pconfig,-amx-complex,+ssse3,+cx16,-avx10.2,+bmi2,+fma,+popcnt,-avxifma,+f16c,-avx512bitalg,+rdpru,+clwb,+mmx,+sse2,+rdseed,-avx512vbmi2,-prefetchi,-amx-movrs,+rdpid,-fma4,-avx512vbmi,+shstk,+vaes,-waitpkg,-sgx,+fxsr,-avx512dq,+sse4a", data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, link_embedded = false, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_triple = "x86_64-unknown-linux-gnu"}>) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %11 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64, 7, 14], vector_common_parallel = [1, 1, 1]>} ins(%cst_0 : f32) outs(%10 : tensor<512x14x14xf32>) -> tensor<512x14x14xf32>
          %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%8, %9 : tensor<512x14x14xf32>, tensor<512x512xf32>) outs(%11 : tensor<512x14x14xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64, 7, 14, 0], distribution = [64, 7, 14, 0], vector_common_parallel = [1, 1, 1, 0], vector_reduction = [0, 0, 0, 8]>} {
          ^bb0(%in: f32, %in_1: f32, %out: f32):
            %14 = arith.mulf %in, %in_1 : f32
            %15 = arith.addf %out, %14 : f32
            linalg.yield %15 : f32
          } -> tensor<512x14x14xf32>
          %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%12 : tensor<512x14x14xf32>) outs(%10 : tensor<512x14x14xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [64, 7, 14], vector_common_parallel = [1, 1, 1]>} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer
  util.initializer {
    ...
    util.global.store %buffer, @main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer
    util.return
  }
  util.func public @main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    %0 = arith.index_cast %arg0 : i32 to index
    %device, %queue_affinity = hal.device.resolve on(#hal.device.affinity<@__device_0>) : !hal.device, i64
    %cmd = hal.command_buffer.create device(%device : !hal.device) mode("OneShot|AllowInlineExecution") categories(Dispatch) affinity(%queue_affinity) : !hal.command_buffer
    %c9084928_i32 = arith.constant 9084928 : i32
    %c7093312_i32 = arith.constant 7093312 : i32
    %main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer = util.global.load @main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer
    ...
    %workgroup_x, %workgroup_y, %workgroup_z = hal.executable.calculate_workgroups device(%device : !hal.device) target(@main_graph$async_dispatch_18::@system_elf_x86_64::@main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32) : index, index, index
    %exe = hal.executable.lookup device(%device : !hal.device) executable(@main_graph$async_dispatch_18) : !hal.executable
    %ordinal = hal.executable.export.ordinal target(@main_graph$async_dispatch_18::@system_elf_x86_64::@main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32) : index
    %c1 = arith.constant 1 : index
    scf.for %arg1 = %c0 to %0 step %c1 {
      hal.command_buffer.dispatch<%cmd : !hal.command_buffer> target(%exe : !hal.executable)[%ordinal] workgroups([%workgroup_x, %workgroup_y, %workgroup_z]) constants([%c9084928_i32, %c7093312_i32]) bindings([
        (%main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer)[%c0, %c10538048], 
        (%main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer)[%c10538240, %c15732224], 
        (%main_graph$async_dispatch_18_system_elf_x86_64_main_graph$async_dispatch_18_matmul_like_512x14x14x512_f32_buffer : !hal.buffer)[%c26270464, %c10538048]
      ]) flags("None")
