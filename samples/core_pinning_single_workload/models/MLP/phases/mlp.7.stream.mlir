...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  stream.executable private @main_graph$async_dispatch_0 {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_0_matmul_like_Dx32x10_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: !stream.binding {stream.alignment = 64 : index}, %arg3: i32, %arg4: i32) {
        ...
        %6 = stream.binding.subspan %arg1[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x10xf32>>
        %7 = iree_tensor_ext.dispatch.workload.ordinal %5, 0 : index
        %8 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x10xf32>>{%7}
        %9 = stream.binding.subspan %arg2[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x32xf32>>{%7}
        %10 = iree_tensor_ext.dispatch.tensor.load %8, offsets = [0, 0], sizes = [%7, 10], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x10xf32>>{%7} -> tensor<?x10xf32>
        %11 = iree_tensor_ext.dispatch.tensor.load %6, offsets = [0, 0], sizes = [32, 10], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x10xf32>> -> tensor<32x10xf32>
        %12 = tensor.empty(%7) : tensor<?x32xf32>
        %13 = linalg.fill ins(%cst_0 : f32) outs(%12 : tensor<?x32xf32>) -> tensor<?x32xf32>
        %14 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "reduction"]} ins(%10, %11 : tensor<?x10xf32>, tensor<32x10xf32>) outs(%13 : tensor<?x32xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
          %16 = arith.mulf %in, %in_1 : f32
          %17 = arith.addf %out, %16 : f32
          linalg.yield %17 : f32
        } -> tensor<?x32xf32>
        %15 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%14, %cst : tensor<?x32xf32>, tensor<32xf32>) outs(%12 : tensor<?x32xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
        ...
        iree_tensor_ext.dispatch.tensor.store %15, %9, offsets = [0, 0], sizes = [%7, 32], strides = [1, 1] : tensor<?x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x32xf32>>{%7}
        return
      }
    }
  }
  stream.executable private @main_graph$async_dispatch_1 {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_1_matmul_like_Dx32x32_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: !stream.binding {stream.alignment = 64 : index}, %arg3: i32, %arg4: i32, %arg5: i32, %arg6: i32) {
        ...
        %10:2 = util.assume.int 
            %4<umin = 0, umax = 1152921504606846848, udiv = 128>, 
            %9<umin = 0, umax = 9007199254740991>
          : index, index
        %11 = stream.binding.subspan %arg1[%c1280] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x32xf32>>
        %12 = iree_tensor_ext.dispatch.workload.ordinal %10#1, 0 : index
        %13 = stream.binding.subspan %arg0[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x32xf32>>{%12}
        %14 = stream.binding.subspan %arg2[%10#0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x32xf32>>{%12}
        %15 = iree_tensor_ext.dispatch.tensor.load %13, offsets = [0, 0], sizes = [%12, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x32xf32>>{%12} -> tensor<?x32xf32>
        %16 = iree_tensor_ext.dispatch.tensor.load %11, offsets = [0, 0], sizes = [32, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x32xf32>> -> tensor<32x32xf32>
        %17 = tensor.empty(%12) : tensor<?x32xf32>
        %18 = linalg.fill ins(%cst_0 : f32) outs(%17 : tensor<?x32xf32>) -> tensor<?x32xf32>
        %19 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "reduction"]} ins(%15, %16 : tensor<?x32xf32>, tensor<32x32xf32>) outs(%18 : tensor<?x32xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
          %21 = arith.mulf %in, %in_1 : f32
          %22 = arith.addf %out, %21 : f32
          linalg.yield %22 : f32
        } -> tensor<?x32xf32>
        %20 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%19, %cst : tensor<?x32xf32>, tensor<32xf32>) outs(%17 : tensor<?x32xf32>) {
        ^bb0(%in: f32, %in_1: f32, %out: f32):
          %21 = arith.addf %in, %in_1 : f32
          %22 = arith.cmpf ugt, %21, %cst_0 : f32
          %23 = arith.select %22, %21, %cst_0 : f32
          linalg.yield %23 : f32
        } -> tensor<?x32xf32>
        iree_tensor_ext.dispatch.tensor.store %20, %14, offsets = [0, 0], sizes = [%12, 32], strides = [1, 1] : tensor<?x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x32xf32>>{%12}
        return
      }
    }
  }
  stream.executable private @main_graph$async_dispatch_2 {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_2_matmul_like_Dx2x32_f32(%arg0: !stream.binding {stream.alignment = 64 : index}, %arg1: !stream.binding {stream.alignment = 64 : index}, %arg2: i32, %arg3: i32, %arg4: i32, %arg5: i32) {
        %c0 = arith.constant 0 : index
        ...
        %cst_0 = arith.constant 0.000000e+00 : f32
        %cst_1 = arith.constant dense<[0.150014639, -0.112291597]> : tensor<2xf32>
        %c32_i64 = arith.constant 32 : i64
        ...
        %10:2 = util.assume.int 
            ...
            %9<umin = 0, umax = 9007199254740991>
          : index, index
        %11 = iree_tensor_ext.dispatch.workload.ordinal %10#1, 0 : index
        %12 = stream.binding.subspan %arg0[%10#0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x32xf32>>{%11}
        %13 = stream.binding.subspan %arg1[%c0] : !stream.binding -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x2xf32>>{%11}
        %14 = iree_tensor_ext.dispatch.tensor.load %12, offsets = [0, 0], sizes = [%11, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x32xf32>>{%11} -> tensor<?x32xf32>
        %15 = tensor.empty(%11) : tensor<?x2xf32>
        %16 = linalg.fill ins(%cst_0 : f32) outs(%15 : tensor<?x2xf32>) -> tensor<?x2xf32>
        %17 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "reduction"]} ins(%14, %cst : tensor<?x32xf32>, tensor<2x32xf32>) outs(%16 : tensor<?x2xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %19 = arith.mulf %in, %in_2 : f32
          %20 = arith.addf %out, %19 : f32
          linalg.yield %20 : f32
        } -> tensor<?x2xf32>
        %18 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel"]} ins(%17, %cst_1 : tensor<?x2xf32>, tensor<2xf32>) outs(%15 : tensor<?x2xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %19 = arith.addf %in, %in_2 : f32
          linalg.yield %19 : f32
        } -> tensor<?x2xf32>
        iree_tensor_ext.dispatch.tensor.store %18, %13, offsets = [0, 0], sizes = [%11, 2], strides = [1, 1] : tensor<?x2xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x2xf32>>{%11}
        return
      }
    }
  }
  ...
  util.func public @main_graph$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %16 = stream.cmd.execute on(#hal.device.affinity<@__device_0>) await(%7) => with(%2 as %arg3: !stream.resource<external>{%1}, %__constant_tensor_32x10xf32 as %arg4: !stream.resource<constant>{%c5376}, %result as %arg5: !stream.resource<external>{%5}, %result_0 as %arg6: !stream.resource<transient>{%6}) {
      stream.cmd.dispatch @main_graph$async_dispatch_0::@main_graph$async_dispatch_0_matmul_like_Dx32x10_f32[%0](%9, %11 : i32, i32) {
      ...
      stream.cmd.dispatch @main_graph$async_dispatch_1::@main_graph$async_dispatch_1_matmul_like_Dx32x32_f32[%0](%13, %15, %9, %11 : i32, i32, i32, i32) {
        ro %arg6[%c0 for %6] : !stream.resource<transient>{%6},
        ro %arg4[%c0 for %c5376] : !stream.resource<constant>{%c5376},
        wo %arg6[%c0 for %6] : !stream.resource<transient>{%6}
      }
      stream.cmd.dispatch @main_graph$async_dispatch_2::@main_graph$async_dispatch_2_matmul_like_Dx2x32_f32[%0](%13, %15, %9, %11 : i32, i32, i32, i32) {
        ro %arg6[%c0 for %6] : !stream.resource<transient>{%6},
