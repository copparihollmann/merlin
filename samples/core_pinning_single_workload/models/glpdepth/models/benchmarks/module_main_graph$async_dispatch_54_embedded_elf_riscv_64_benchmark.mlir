module {
  ...
  hal.executable private @main_graph$async_dispatch_54 {
    hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
      hal.executable.export public @main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack ordinal(0) layout(#hal.pipeline.layout<constants = 1, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) count(%arg0: !hal.device) -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %3 = hal.interface.binding.subspan layout(<constants = 1, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%c1003520) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<784x64xf32>>
          %4 = hal.interface.binding.subspan layout(<constants = 1, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(1) alignment(64) offset(%2) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x2x32xf32>>
          %5 = hal.interface.binding.subspan layout(<constants = 1, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, ReadOnly>, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(2) alignment(64) offset(%c401408) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x25x32x32x1xf32>>
          ...
          %10 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 112, 32], vector_common_parallel = [1, 32, 1]>} ins(%cst : f32) outs(%9 : tensor<2x784x32xf32>) -> tensor<2x784x32xf32>
          %11 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d0, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%6, %7 : tensor<784x64xf32>, tensor<64x2x32xf32>) outs(%10 : tensor<2x784x32xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 112, 32, 0], distribution = [1, 128, 32, 0], vector_common_parallel = [1, 32, 1, 0], vector_reduction = [0, 0, 0, 1]>} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %13 = arith.mulf %in, %in_0 : f32
            %14 = arith.addf %out, %13 : f32
            linalg.yield %14 : f32
          } -> tensor<2x784x32xf32>
          %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%11 : tensor<2x784x32xf32>) outs(%9 : tensor<2x784x32xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 112, 32], vector_common_parallel = [1, 32, 1]>} {
          ^bb0(%in: f32, %out: f32):
            %13 = arith.addf %in, %cst : f32
            linalg.yield %13 : f32
          } -> tensor<2x784x32xf32>
          %pack = linalg.pack %12 padding_value(%cst : f32) outer_dims_perm = [0, 1, 2] inner_dims_pos = [1, 2] inner_tiles = [32, 1] into %8 {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 3, 32], vector_common_parallel = [1, 1, 1]>} : tensor<2x784x32xf32> -> tensor<2x25x32x32x1xf32>
  ...
  util.global private mutable @main_graph$async_dispatch_54_embedded_elf_riscv_64_main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_54_embedded_elf_riscv_64_main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_54_embedded_elf_riscv_64_main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack_buffer = util.global.load @main_graph$async_dispatch_54_embedded_elf_riscv_64_main_graph$async_dispatch_54_matmul_like_2x784x32x64_f32_pack_buffer : !hal.buffer
