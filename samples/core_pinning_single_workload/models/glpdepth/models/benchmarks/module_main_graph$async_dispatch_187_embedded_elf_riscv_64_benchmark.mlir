module {
  ...
  hal.executable private @main_graph$async_dispatch_187 {
    hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_187_reduction_49x256_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %7 = linalg.fill {lowering_config = #iree_cpu.lowering_config<vector_inner_parallel = [1]>} ins(%cst : f32) outs(%6 : tensor<49xf32>) -> tensor<49xf32>
          %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%3, %4 : tensor<49x256xf32>, tensor<49x256xf32>) outs(%7 : tensor<49xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<vector_inner_parallel = [1, 0], vector_reduction = [0, 8]>} {
          ...
          %9 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%3, %4, %8 : tensor<49x256xf32>, tensor<49x256xf32>, tensor<49xf32>) outs(%7 : tensor<49xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<distribution = [1, 0], vector_reduction = [0, 8]>} {
