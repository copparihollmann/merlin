module {
  ...
  hal.executable private @main_graph$async_dispatch_11 {
    hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
      hal.executable.export public @main_graph$async_dispatch_11_unpack_f32 ordinal(0) layout(#hal.pipeline.layout<constants = 2, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) count(%arg0: !hal.device) -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @main_graph$async_dispatch_11_unpack_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDataTiling>} {
          ...
          %4:2 = util.assume.int 
              %2[<umin = 821632, umax = 821632, udiv = 821632>, <umin = 1204224, umax = 1204224, udiv = 1204224>], 
              %3[<umin = 1436288, umax = 1436288, udiv = 1436288>, <umin = 420224, umax = 420224, udiv = 420224>]
            : index, index
          %5 = hal.interface.binding.subspan layout(<constants = 2, bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%4#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<7x98x7x32xf32>>
          ...
          %7 = iree_tensor_ext.dispatch.tensor.load %5, offsets = [0, 0, 0, 0], sizes = [7, 98, 7, 32], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<7x98x7x32xf32>> -> tensor<7x98x7x32xf32>
          %8 = tensor.empty() : tensor<3136x49xf32>
          %unpack = linalg.unpack %7 outer_dims_perm = [1, 0] inner_dims_pos = [1, 0] inner_tiles = [7, 32] into %8 {lowering_config = #iree_cpu.lowering_config<distribution = [64, 49], vector_common_parallel = [32, 7]>} : tensor<7x98x7x32xf32> -> tensor<3136x49xf32>
          iree_tensor_ext.dispatch.tensor.store %unpack, %6, offsets = [0, 0], sizes = [3136, 49], strides = [1, 1] : tensor<3136x49xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x49xf32>>
          return
        }
      }
    }
  }
  util.global private mutable @main_graph$async_dispatch_11_embedded_elf_riscv_64_main_graph$async_dispatch_11_unpack_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_11_embedded_elf_riscv_64_main_graph$async_dispatch_11_unpack_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_11_embedded_elf_riscv_64_main_graph$async_dispatch_11_unpack_f32_buffer = util.global.load @main_graph$async_dispatch_11_embedded_elf_riscv_64_main_graph$async_dispatch_11_unpack_f32_buffer : !hal.buffer
