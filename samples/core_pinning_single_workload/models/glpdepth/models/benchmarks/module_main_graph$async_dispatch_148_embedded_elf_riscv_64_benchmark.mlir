module {
  ...
  hal.executable private @main_graph$async_dispatch_148 {
    hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %14 = linalg.fill {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 7, 32], vector_common_parallel = [1, 7, 1]>} ins(%cst : f32) outs(%13 : tensor<8x49x32xf32>) -> tensor<8x49x32xf32>
          %15 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d0, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%10, %11 : tensor<49x256xf32>, tensor<256x8x32xf32>) outs(%14 : tensor<8x49x32xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 7, 32, 0], distribution = [1, 7, 32, 0], vector_common_parallel = [1, 7, 1, 0], vector_reduction = [0, 0, 0, 1]>} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %17 = arith.mulf %in, %in_0 : f32
            %18 = arith.addf %out, %17 : f32
            linalg.yield %18 : f32
          } -> tensor<8x49x32xf32>
          %16 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%15 : tensor<8x49x32xf32>) outs(%13 : tensor<8x49x32xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 7, 32], vector_common_parallel = [1, 7, 1]>} {
          ^bb0(%in: f32, %out: f32):
            %17 = arith.addf %in, %cst : f32
            linalg.yield %17 : f32
          } -> tensor<8x49x32xf32>
          %pack = linalg.pack %16 outer_dims_perm = [0, 1, 2] inner_dims_pos = [1, 2] inner_tiles = [7, 1] into %12 {lowering_config = #iree_cpu.lowering_config<cache_parallel = [1, 1, 32], vector_common_parallel = [1, 1, 1]>} : tensor<8x49x32xf32> -> tensor<8x7x32x7x1xf32>
