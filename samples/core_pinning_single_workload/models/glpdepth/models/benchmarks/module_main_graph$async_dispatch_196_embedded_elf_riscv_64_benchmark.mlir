module {
  ...
  hal.executable private @main_graph$async_dispatch_196 {
    hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
      hal.executable.export public @main_graph$async_dispatch_196_elementwise_broadcast_50176_f32 ordinal(0) layout(#hal.pipeline.layout<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) count(%arg0: !hal.device) -> (index, index, index) {
        %x, %y, %z = iree_tensor_ext.dispatch.workgroup_count_from_slice()
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @main_graph$async_dispatch_196_elementwise_broadcast_50176_f32() attributes {translation_info = #iree_codegen.translation_info<pipeline = CPUDoubleTilingExpert, {enable_loop_peeling}>} {
          ...
          %c1404928 = arith.constant 1404928 : index
          %0 = hal.interface.binding.subspan layout(<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%c639744) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x64x14x14xf32>>
          %1 = hal.interface.binding.subspan layout(<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(1) alignment(64) offset(%c1404928) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<50176xf32>>
          %2 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [0, 0, 0, 0], sizes = [1, 64, 14, 14], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1x64x14x14xf32>> -> tensor<1x64x14x14xf32>
          %3 = tensor.empty() : tensor<50176xf32>
          %4 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} outs(%3 : tensor<50176xf32>) attrs =  {lowering_config = #iree_cpu.lowering_config<distribution = [4096], vector_common_parallel = [8]>} {
          ^bb0(%out: f32):
            %5 = linalg.index 0 : index
            %6 = arith.remsi %5, %c28 : index
            %7 = arith.divsi %5, %c28 : index
            %8 = arith.remsi %7, %c28 : index
            %9 = arith.divsi %7, %c28 : index
            ...
            %extracted = tensor.extract %2[%c0, %9, %28, %36] : tensor<1x64x14x14xf32>
            %extracted_4 = tensor.extract %2[%c0, %9, %28, %39] : tensor<1x64x14x14xf32>
            %extracted_5 = tensor.extract %2[%c0, %9, %31, %36] : tensor<1x64x14x14xf32>
            %extracted_6 = tensor.extract %2[%c0, %9, %31, %39] : tensor<1x64x14x14xf32>
          ...
          iree_tensor_ext.dispatch.tensor.store %4, %1, offsets = [0], sizes = [50176], strides = [1] : tensor<50176xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<50176xf32>>
          return
        }
      }
    }
  }
  util.global private mutable @main_graph$async_dispatch_196_embedded_elf_riscv_64_main_graph$async_dispatch_196_elementwise_broadcast_50176_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_196_embedded_elf_riscv_64_main_graph$async_dispatch_196_elementwise_broadcast_50176_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_196_embedded_elf_riscv_64_main_graph$async_dispatch_196_elementwise_broadcast_50176_f32_buffer = util.global.load @main_graph$async_dispatch_196_embedded_elf_riscv_64_main_graph$async_dispatch_196_elementwise_broadcast_50176_f32_buffer : !hal.buffer
    %c0 = arith.constant 0 : index
