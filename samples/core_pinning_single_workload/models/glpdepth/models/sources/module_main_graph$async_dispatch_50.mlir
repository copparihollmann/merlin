hal.executable public @main_graph$async_dispatch_50 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_50_reduction_3136x32_f32() {
        ...
        %7 = tensor.empty() : tensor<3136x32xf32>
        %8 = linalg.fill ins(%cst : f32) outs(%6 : tensor<3136xf32>) -> tensor<3136xf32>
        %9 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%3, %4 : tensor<3136x32xf32>, tensor<3136x32xf32>) outs(%7 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %14 = arith.addf %in_2, %cst : f32
          %15 = arith.addf %in, %14 : f32
          linalg.yield %15 : f32
        } -> tensor<3136x32xf32>
        %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%9 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %14 = arith.addf %in, %out : f32
          linalg.yield %14 : f32
        } -> tensor<3136xf32>
        %11 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%9, %10 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%7 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %14 = arith.divf %in_2, %cst_0 : f32
          %15 = arith.subf %in, %14 : f32
          linalg.yield %15 : f32
        } -> tensor<3136x32xf32>
        %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%11 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %14 = arith.mulf %in, %in : f32
          %15 = arith.addf %14, %out : f32
          linalg.yield %15 : f32
        } -> tensor<3136xf32>
        %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%11, %12 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%5 : tensor<32x3136xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %14 = arith.divf %in_2, %cst_0 : f32
        ...
        iree_tensor_ext.dispatch.tensor.store %13, %2, offsets = [0, 0], sizes = [32, 3136], strides = [1, 1] : tensor<32x3136xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<32x3136xf32>>
        return
      }
    }
  }
