hal.executable public @main_graph$async_dispatch_61 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store() {
        ...
        %4 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%2 : tensor<2x784x49xf32>) outs(%3 : tensor<2x784x49xf32>) {
        ^bb0(%in: f32, %out: f32):
          %6 = arith.mulf %in, %cst : f32
          linalg.yield %6 : f32
        } -> tensor<2x784x49xf32>
        %5 = linalg.softmax dimension(2) ins(%4 : tensor<2x784x49xf32>) outs(%3 : tensor<2x784x49xf32>) -> tensor<2x784x49xf32>
        iree_tensor_ext.dispatch.tensor.store %5, %1, offsets = [0, 0, 0], sizes = [2, 784, 49], strides = [1, 1, 1] : tensor<2x784x49xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x784x49xf32>>
        return
      }
    }
  }
