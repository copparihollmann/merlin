hal.executable public @main_graph$async_dispatch_2 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_2_reduction_3136x32_f32() {
        ...
        %8 = linalg.fill ins(%cst_1 : f32) outs(%7 : tensor<3136xf32>) -> tensor<3136xf32>
        %9 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%4 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %17 = arith.addf %in, %out : f32
          linalg.yield %17 : f32
        } -> tensor<3136xf32>
        %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%5, %9 : tensor<32x3136xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %17 = arith.divf %in_3, %cst : f32
          %18 = arith.subf %in, %17 : f32
          linalg.yield %18 : f32
        } -> tensor<3136x32xf32>
        %11 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%10 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %17 = arith.mulf %in, %in : f32
          %18 = arith.addf %17, %out : f32
          linalg.yield %18 : f32
        } -> tensor<3136xf32>
        %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%10, %11 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %17 = arith.divf %in_3, %cst : f32
        ...
        %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%12 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %17 = arith.addf %in, %out : f32
          linalg.yield %17 : f32
        } -> tensor<3136xf32>
        %14 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%12, %13 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %17 = arith.divf %in_3, %cst : f32
          %18 = arith.subf %in, %17 : f32
          linalg.yield %18 : f32
        } -> tensor<3136x32xf32>
        %15 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%14 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) {
        ^bb0(%in: f32, %out: f32):
          %17 = arith.mulf %in, %in : f32
          %18 = arith.addf %17, %out : f32
          linalg.yield %18 : f32
        } -> tensor<3136xf32>
        %16 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%14, %15 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) {
        ^bb0(%in: f32, %in_3: f32, %out: f32):
          %17 = arith.divf %in_3, %cst : f32
        ...
        iree_tensor_ext.dispatch.tensor.store %12, %2, offsets = [0, 0], sizes = [3136, 32], strides = [1, 1] : tensor<3136x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x32xf32>>
        iree_tensor_ext.dispatch.tensor.store %16, %3, offsets = [0, 0], sizes = [3136, 32], strides = [1, 1] : tensor<3136x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x32xf32>>
        return
      }
    }
  }
