hal.executable public @main_graph$async_dispatch_56 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_56_reduction_49x64_f32() {
        ...
        %7 = linalg.fill ins(%cst_1 : f32) outs(%6 : tensor<49xf32>) -> tensor<49xf32>
        %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%3 : tensor<49x64xf32>) outs(%7 : tensor<49xf32>) {
        ^bb0(%in: f32, %out: f32):
          %12 = arith.addf %in, %out : f32
          linalg.yield %12 : f32
        } -> tensor<49xf32>
        %9 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%4, %8 : tensor<64x49xf32>, tensor<49xf32>) outs(%5 : tensor<49x64xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %12 = arith.divf %in_2, %cst : f32
          %13 = arith.subf %in, %12 : f32
          linalg.yield %13 : f32
        } -> tensor<49x64xf32>
        %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%9 : tensor<49x64xf32>) outs(%7 : tensor<49xf32>) {
        ^bb0(%in: f32, %out: f32):
          %12 = arith.mulf %in, %in : f32
          %13 = arith.addf %12, %out : f32
          linalg.yield %13 : f32
        } -> tensor<49xf32>
        %11 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%9, %10 : tensor<49x64xf32>, tensor<49xf32>) outs(%5 : tensor<49x64xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %12 = arith.divf %in_2, %cst : f32
        ...
        iree_tensor_ext.dispatch.tensor.store %11, %2, offsets = [0, 0], sizes = [49, 64], strides = [1, 1] : tensor<49x64xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<49x64xf32>>
        return
      }
    }
  }
