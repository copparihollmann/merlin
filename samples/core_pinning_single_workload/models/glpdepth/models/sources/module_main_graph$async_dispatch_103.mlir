hal.executable public @main_graph$async_dispatch_103 {
  hal.executable.variant public @embedded_elf_riscv_64 target(<"llvm-cpu", "embedded-elf-riscv_64", {cpu = "spacemit-x60", cpu_features = "+m,+a,+f,+d,+c,+v,+zvl256b,+zba,+zbb,+zbc,+zbs,+zicbom,+zicboz,+zicbop,+zihintpause", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", iree.encoding.resolver = #iree_cpu.cpu_encoding_resolver<>, loop_vectorization = true, max_stack_allocation_size = 32768 : i64, native_vector_size = 32 : i64, target_abi = "lp64d", target_triple = "riscv64-unknown-unknown-eabi-elf"}>) {
    ...
    builtin.module {
      func.func @main_graph$async_dispatch_103_reduction_49x160_f32() {
        ...
        %15 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%10 : tensor<49x160xf32>) outs(%14 : tensor<49xf32>) {
        ^bb0(%in: f32, %out: f32):
          %19 = arith.addf %in, %out : f32
          linalg.yield %19 : f32
        } -> tensor<49xf32>
        %16 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d1, d0)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%11, %15 : tensor<160x49xf32>, tensor<49xf32>) outs(%12 : tensor<49x160xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %19 = arith.divf %in_2, %cst_1 : f32
          %20 = arith.subf %in, %19 : f32
          linalg.yield %20 : f32
        } -> tensor<49x160xf32>
        %17 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]} ins(%16 : tensor<49x160xf32>) outs(%14 : tensor<49xf32>) {
        ^bb0(%in: f32, %out: f32):
          %19 = arith.mulf %in, %in : f32
          %20 = arith.addf %19, %out : f32
          linalg.yield %20 : f32
        } -> tensor<49xf32>
        %18 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%16, %17 : tensor<49x160xf32>, tensor<49xf32>) outs(%12 : tensor<49x160xf32>) {
        ^bb0(%in: f32, %in_2: f32, %out: f32):
          %19 = arith.divf %in_2, %cst_1 : f32
        ...
        iree_tensor_ext.dispatch.tensor.store %18, %9, offsets = [0, 0], sizes = [49, 160], strides = [1, 1] : tensor<49x160xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<49x160xf32>>
        return
      }
    }
  }
