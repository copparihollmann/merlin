#config = #iree_cpu.lowering_config<vector_inner_parallel = [7]>
#config1 = #iree_cpu.lowering_config<vector_inner_parallel = [7, 0], vector_reduction = [0, 8]>
#config2 = #iree_cpu.lowering_config<distribution = [14, 0], vector_reduction = [0, 8]>
#config3 = #iree_cpu.lowering_config<vector_inner_parallel = [7, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_144 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_144_reduction_196x160_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c652288) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<196x160xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1003520) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<196x160xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c401408) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<160x196xf32>>
          ...
          %7 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%6 : tensor<196xf32>) -> tensor<196xf32>
          %8 = linalg.generic {indexing_maps = [#map, #map, #map1], iterator_types = ["parallel", "reduction"]} ins(%3, %4 : tensor<196x160xf32>, tensor<196x160xf32>) outs(%7 : tensor<196xf32>) attrs =  {lowering_config = #config1} {
          ...
          %9 = linalg.generic {indexing_maps = [#map, #map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%3, %4, %8 : tensor<196x160xf32>, tensor<196x160xf32>, tensor<196xf32>) outs(%7 : tensor<196xf32>) attrs =  {lowering_config = #config2} {
          ...
          %10 = linalg.generic {indexing_maps = [#map2, #map2, #map3, #map3, #map], iterator_types = ["parallel", "parallel"]} ins(%3, %4, %8, %9 : tensor<196x160xf32>, tensor<196x160xf32>, tensor<196xf32>, tensor<196xf32>) outs(%5 : tensor<160x196xf32>) attrs =  {lowering_config = #config3} {
  ...
  util.global private mutable @main_graph$async_dispatch_144_embedded_elf_riscv_64_main_graph$async_dispatch_144_reduction_196x160_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_144_embedded_elf_riscv_64_main_graph$async_dispatch_144_reduction_196x160_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_144_embedded_elf_riscv_64_main_graph$async_dispatch_144_reduction_196x160_f32_buffer = util.global.load @main_graph$async_dispatch_144_embedded_elf_riscv_64_main_graph$async_dispatch_144_reduction_196x160_f32_buffer : !hal.buffer
