#config = #iree_cpu.lowering_config<distribution = [2, 32, 49], vector_common_parallel = [1, 1, 7]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_58 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_58_elementwise_broadcast_2x32x49_f32_pack() attributes {translation_info = #translation} {
          %c0 = arith.constant 0 : index
          %c606208 = arith.constant 606208 : index
          %c631296 = arith.constant 631296 : index
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c606208) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x1x2x49x32xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c631296) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x7x32x7x1xf32>>
          ...
          %5 = linalg.generic {indexing_maps = [#map], iterator_types = ["parallel", "parallel", "parallel"]} outs(%4 : tensor<2x32x49xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%out: f32):
          ...
          %pack = linalg.pack %5 outer_dims_perm = [0, 2, 1] inner_dims_pos = [2, 1] inner_tiles = [7, 1] into %3 {lowering_config = #config1} : tensor<2x32x49xf32> -> tensor<2x7x32x7x1xf32>
