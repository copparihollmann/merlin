...
#config4 = #iree_cpu.lowering_config<distribution = [28, 0], vector_common_parallel = [7, 0], vector_reduction = [0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_53 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_53_reduction_784x64_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c602112) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<784x64xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c401408) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x784xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c802816) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<784x64xf32>>
          %3 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1003520) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<784x64xf32>>
          ...
          %8 = linalg.fill {lowering_config = #config} ins(%cst_1 : f32) outs(%7 : tensor<784xf32>) -> tensor<784xf32>
          %9 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%4 : tensor<784x64xf32>) outs(%8 : tensor<784xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %15 = arith.addf %in, %out : f32
            linalg.yield %15 : f32
          } -> tensor<784xf32>
          %10 = linalg.generic {indexing_maps = [#map2, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%5, %9 : tensor<64x784xf32>, tensor<784xf32>) outs(%8 : tensor<784xf32>) attrs =  {lowering_config = #config2} {
          ...
          %11 = linalg.generic {indexing_maps = [#map2, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%5, %9, %10 : tensor<64x784xf32>, tensor<784xf32>, tensor<784xf32>) outs(%6 : tensor<784x64xf32>) attrs =  {lowering_config = #config3} {
          ...
          %12 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%11 : tensor<784x64xf32>) outs(%8 : tensor<784xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %15 = arith.addf %in, %out : f32
            linalg.yield %15 : f32
          } -> tensor<784xf32>
          %13 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%11, %12 : tensor<784x64xf32>, tensor<784xf32>) outs(%8 : tensor<784xf32>) attrs =  {lowering_config = #config4} {
          ^bb0(%in: f32, %in_3: f32, %out: f32):
            %15 = arith.divf %in_3, %cst : f32
            %16 = arith.subf %in, %15 : f32
            %17 = arith.mulf %16, %16 : f32
          ...
          %14 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%11, %12, %13 : tensor<784x64xf32>, tensor<784xf32>, tensor<784xf32>) outs(%6 : tensor<784x64xf32>) attrs =  {lowering_config = #config3} {
          ^bb0(%in: f32, %in_3: f32, %in_4: f32, %out: f32):
            %15 = arith.divf %in_3, %cst : f32
            %16 = arith.subf %in, %15 : f32
            %17 = arith.divf %in_4, %cst : f32
