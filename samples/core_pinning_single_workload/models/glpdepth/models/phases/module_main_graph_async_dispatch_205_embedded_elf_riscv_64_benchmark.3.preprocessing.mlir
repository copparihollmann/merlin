#config = #iree_cpu.lowering_config<distribution = [16, 28, 56], vector_common_parallel = [1, 1, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_205 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_205_slow_memcpy() attributes {translation_info = #translation} {
          %c2007040 = arith.constant 2007040 : index
          %c3612672 = arith.constant 3612672 : index
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c2007040) flags("ReadOnly|Indirect") : memref<128x56x56xf32, strided<[3136, 56, 1], offset: ?>>
          %assume_align = memref.assume_alignment %0, 64 : memref<128x56x56xf32, strided<[3136, 56, 1], offset: ?>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c3612672) flags(Indirect) : memref<128x58x58xf32, strided<[3364, 58, 1], offset: ?>>
          ...
          linalg.generic {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%assume_align : memref<128x56x56xf32, strided<[3136, 56, 1], offset: ?>>) outs(%subview : memref<128x56x56xf32, strided<[3364, 58, 1], offset: ?>>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
            linalg.yield %in : f32
          }
          return
  ...
  util.global private mutable @main_graph$async_dispatch_205_embedded_elf_riscv_64_main_graph$async_dispatch_205_slow_memcpy_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_205_embedded_elf_riscv_64_main_graph$async_dispatch_205_slow_memcpy(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_205_embedded_elf_riscv_64_main_graph$async_dispatch_205_slow_memcpy_buffer = util.global.load @main_graph$async_dispatch_205_embedded_elf_riscv_64_main_graph$async_dispatch_205_slow_memcpy_buffer : !hal.buffer
