...
#config1 = #iree_cpu.lowering_config<distribution = [1, 14, 28, 0, 0, 0], vector_common_parallel = [1, 1, 8, 0, 0, 0], vector_reduction = [0, 0, 0, 1, 1, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_208 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_208_conv_2x56x56x32x3x3_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c602112) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x58x58xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c8652544) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x32x3x3xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1032704) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x56x56xf32>>
          ...
          %6 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%5 : tensor<2x56x56xf32>) -> tensor<2x56x56xf32>
          %7 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%3, %4 : tensor<32x58x58xf32>, tensor<2x32x3x3xf32>) outs(%6 : tensor<2x56x56xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
            %9 = arith.mulf %in, %in_2 : f32
            %10 = arith.addf %out, %9 : f32
            linalg.yield %10 : f32
          } -> tensor<2x56x56xf32>
          %8 = linalg.generic {indexing_maps = [#map3, #map4, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%7, %cst_1 : tensor<2x56x56xf32>, tensor<2xf32>) outs(%5 : tensor<2x56x56xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_208_embedded_elf_riscv_64_main_graph$async_dispatch_208_conv_2x56x56x32x3x3_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_208_embedded_elf_riscv_64_main_graph$async_dispatch_208_conv_2x56x56x32x3x3_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_208_embedded_elf_riscv_64_main_graph$async_dispatch_208_conv_2x56x56x32x3x3_f32_buffer = util.global.load @main_graph$async_dispatch_208_embedded_elf_riscv_64_main_graph$async_dispatch_208_conv_2x56x56x32x3x3_f32_buffer : !hal.buffer
