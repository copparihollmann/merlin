...
#config1 = #iree_cpu.lowering_config<distribution = [32, 32, 0, 0, 0], vector_common_parallel = [1, 8, 0, 0, 0], vector_reduction = [0, 0, 1, 1, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_213 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_213_conv_224x224x64x3x3_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c19096576) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x226x226xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c13847424) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x3x3xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c0) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<224x224xf32>>
          ...
          %6 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%5 : tensor<224x224xf32>) -> tensor<224x224xf32>
          %7 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "reduction", "reduction", "reduction"]} ins(%3, %4 : tensor<64x226x226xf32>, tensor<64x3x3xf32>) outs(%6 : tensor<224x224xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_3: f32, %out: f32):
            %9 = arith.mulf %in, %in_3 : f32
            %10 = arith.addf %out, %9 : f32
            linalg.yield %10 : f32
          } -> tensor<224x224xf32>
          %8 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel"]} ins(%7 : tensor<224x224xf32>) outs(%5 : tensor<224x224xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
