#config = #iree_cpu.lowering_config<cache_parallel = [2, 7, 32], vector_common_parallel = [1, 7, 16]>
#config1 = #iree_cpu.lowering_config<cache_parallel = [2, 7, 32, 0], distribution = [2, 7, 32, 0], vector_common_parallel = [1, 7, 16, 0], vector_reduction = [0, 0, 0, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_149 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_149_matmul_like_16x49x32x256_f32() attributes {translation_info = #translation} {
          ...
          %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%6#1) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<256x16x32xf32>>
          %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%6#2) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<16x49x32xf32>>
          ...
          %13 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%12 : tensor<16x49x32xf32>) -> tensor<16x49x32xf32>
          %14 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%10, %11 : tensor<49x256xf32>, tensor<256x16x32xf32>) outs(%13 : tensor<16x49x32xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %16 = arith.mulf %in, %in_0 : f32
            %17 = arith.addf %out, %16 : f32
            linalg.yield %17 : f32
          } -> tensor<16x49x32xf32>
          %15 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%14 : tensor<16x49x32xf32>) outs(%12 : tensor<16x49x32xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_149_embedded_elf_riscv_64_main_graph$async_dispatch_149_matmul_like_16x49x32x256_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_149_embedded_elf_riscv_64_main_graph$async_dispatch_149_matmul_like_16x49x32x256_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_149_embedded_elf_riscv_64_main_graph$async_dispatch_149_matmul_like_16x49x32x256_f32_buffer = util.global.load @main_graph$async_dispatch_149_embedded_elf_riscv_64_main_graph$async_dispatch_149_matmul_like_16x49x32x256_f32_buffer : !hal.buffer
