...
#config3 = #iree_cpu.lowering_config<distribution = [14, 0], vector_common_parallel = [7, 0], vector_reduction = [0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_116 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_116_reduction_196x160_f32() attributes {translation_info = #translation} {
          ...
          %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%8#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<196x160xf32>>
          %10 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%8#1) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<196x160xf32>>
          %11 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%8#2) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<196x160xf32>>
          %12 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%8#3) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<196x160xf32>>
          ...
          %17 = linalg.fill {lowering_config = #config} ins(%cst_1 : f32) outs(%15 : tensor<196xf32>) -> tensor<196xf32>
          %18 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%13, %14 : tensor<196x160xf32>, tensor<196x160xf32>) outs(%16 : tensor<196x160xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
            %22 = arith.addf %in_2, %cst_1 : f32
            %23 = arith.addf %in, %22 : f32
            linalg.yield %23 : f32
          } -> tensor<196x160xf32>
          %19 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%18 : tensor<196x160xf32>) outs(%17 : tensor<196xf32>) attrs =  {lowering_config = #config2} {
          ^bb0(%in: f32, %out: f32):
            %22 = arith.addf %in, %out : f32
            linalg.yield %22 : f32
          } -> tensor<196xf32>
          %20 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%18, %19 : tensor<196x160xf32>, tensor<196xf32>) outs(%17 : tensor<196xf32>) attrs =  {lowering_config = #config3} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
          ...
          %21 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%18, %19, %20 : tensor<196x160xf32>, tensor<196xf32>, tensor<196xf32>) outs(%16 : tensor<196x160xf32>) attrs =  {lowering_config = #config1} {
