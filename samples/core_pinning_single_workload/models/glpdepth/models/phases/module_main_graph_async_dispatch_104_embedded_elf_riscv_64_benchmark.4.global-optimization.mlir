#config = #iree_cpu.lowering_config<cache_parallel = [1, 49, 32], vector_common_parallel = [1, 7, 16]>
#config1 = #iree_cpu.lowering_config<cache_parallel = [1, 49, 32, 0], distribution = [1, 49, 32, 0], vector_common_parallel = [1, 7, 16, 0], vector_reduction = [0, 0, 0, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_104 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_104_matmul_like_10x49x32x160_f32() attributes {translation_info = #translation} {
          ...
          %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%6#1) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<160x10x32xf32>>
          %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%6#2) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<10x49x32xf32>>
          ...
          %13 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%12 : tensor<10x49x32xf32>) -> tensor<10x49x32xf32>
          %14 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%10, %11 : tensor<49x160xf32>, tensor<160x10x32xf32>) outs(%13 : tensor<10x49x32xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %16 = arith.mulf %in, %in_0 : f32
            %17 = arith.addf %out, %16 : f32
            linalg.yield %17 : f32
          } -> tensor<10x49x32xf32>
          %15 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%14 : tensor<10x49x32xf32>) outs(%12 : tensor<10x49x32xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
