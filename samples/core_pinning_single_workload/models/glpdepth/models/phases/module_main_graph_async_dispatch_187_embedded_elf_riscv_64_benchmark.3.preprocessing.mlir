#config = #iree_cpu.lowering_config<vector_inner_parallel = [1]>
#config1 = #iree_cpu.lowering_config<vector_inner_parallel = [1, 0], vector_reduction = [0, 8]>
#config2 = #iree_cpu.lowering_config<distribution = [1, 0], vector_reduction = [0, 8]>
#config3 = #iree_cpu.lowering_config<vector_inner_parallel = [1, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_187 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_187_reduction_49x256_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c577024) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x256xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c627200) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x256xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c526848) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<256x49xf32>>
          ...
          %7 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%6 : tensor<49xf32>) -> tensor<49xf32>
          %8 = linalg.generic {indexing_maps = [#map, #map, #map1], iterator_types = ["parallel", "reduction"]} ins(%3, %4 : tensor<49x256xf32>, tensor<49x256xf32>) outs(%7 : tensor<49xf32>) attrs =  {lowering_config = #config1} {
          ...
          %9 = linalg.generic {indexing_maps = [#map, #map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%3, %4, %8 : tensor<49x256xf32>, tensor<49x256xf32>, tensor<49xf32>) outs(%7 : tensor<49xf32>) attrs =  {lowering_config = #config2} {
          ...
          %10 = linalg.generic {indexing_maps = [#map2, #map2, #map3, #map3, #map], iterator_types = ["parallel", "parallel"]} ins(%3, %4, %8, %9 : tensor<49x256xf32>, tensor<49x256xf32>, tensor<49xf32>, tensor<49xf32>) outs(%5 : tensor<256x49xf32>) attrs =  {lowering_config = #config3} {
          ...
          iree_tensor_ext.dispatch.tensor.store %10, %2, offsets = [0, 0], sizes = [256, 49], strides = [1, 1] : tensor<256x49xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<256x49xf32>>
          return
        }
      }
    }
  }
  util.global private mutable @main_graph$async_dispatch_187_embedded_elf_riscv_64_main_graph$async_dispatch_187_reduction_49x256_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_187_embedded_elf_riscv_64_main_graph$async_dispatch_187_reduction_49x256_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_187_embedded_elf_riscv_64_main_graph$async_dispatch_187_reduction_49x256_f32_buffer = util.global.load @main_graph$async_dispatch_187_embedded_elf_riscv_64_main_graph$async_dispatch_187_reduction_49x256_f32_buffer : !hal.buffer
