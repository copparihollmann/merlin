...
#config2 = #iree_cpu.lowering_config<distribution = [2, 28, 0], vector_common_parallel = [1, 8, 0], vector_reduction = [0, 0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_61 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1317120) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x784x49xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c1003520) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x784x49xf32>>
          ...
          %5 = linalg.fill {lowering_config = #config} ins(%cst_0 : f32) outs(%4 : tensor<2x784xf32>) -> tensor<2x784xf32>
          %6 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "reduction"]} ins(%2 : tensor<2x784x49xf32>) outs(%5 : tensor<2x784xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %10 = arith.mulf %in, %cst_1 : f32
            %11 = arith.maxnumf %10, %out : f32
            linalg.yield %11 : f32
          } -> tensor<2x784xf32>
          %7 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%4 : tensor<2x784xf32>) -> tensor<2x784xf32>
          %8 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "parallel", "reduction"]} ins(%2, %6 : tensor<2x784x49xf32>, tensor<2x784xf32>) outs(%7 : tensor<2x784xf32>) attrs =  {lowering_config = #config2} {
          ...
          %9 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%2, %6, %8 : tensor<2x784x49xf32>, tensor<2x784xf32>, tensor<2x784xf32>) outs(%3 : tensor<2x784x49xf32>) attrs =  {lowering_config = #config3} {
  ...
  util.global private mutable @main_graph$async_dispatch_61_embedded_elf_riscv_64_main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_61_embedded_elf_riscv_64_main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_61_embedded_elf_riscv_64_main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store_buffer = util.global.load @main_graph$async_dispatch_61_embedded_elf_riscv_64_main_graph$async_dispatch_61_softmax_2x784x49xf32_dispatch_tensor_store_buffer : !hal.buffer
