#config = #iree_cpu.lowering_config<cache_parallel = [32, 14, 28], vector_common_parallel = [1, 8, 32]>
#config1 = #iree_cpu.lowering_config<cache_parallel = [32, 14, 28, 0], distribution = [32, 14, 28, 0], vector_common_parallel = [1, 8, 32, 0], vector_reduction = [0, 0, 0, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_70 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_70_matmul_like_256x28x28x64_f32() attributes {translation_info = #translation} {
          ...
          %4 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%2) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x256xf32>>
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1204224) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<readwrite:tensor<256x30x30xf32>>
          ...
          %9 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%8 : tensor<256x28x28xf32>) -> tensor<256x28x28xf32>
          %10 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%6, %7 : tensor<28x28x64xf32>, tensor<64x256xf32>) outs(%9 : tensor<256x28x28xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %12 = arith.mulf %in, %in_0 : f32
            %13 = arith.addf %out, %12 : f32
            linalg.yield %13 : f32
          } -> tensor<256x28x28xf32>
          %11 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%10 : tensor<256x28x28xf32>) outs(%8 : tensor<256x28x28xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
