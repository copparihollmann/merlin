...
module {
  ...
  hal.executable private @main_graph$async_dispatch_103 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_103_reduction_49x160_f32() attributes {translation_info = #translation} {
          ...
          %6:3 = util.assume.int 
            ...
            : index, index, index
          %7 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%6#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x160xf32>>
          %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%6#1) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<160x49xf32>>
          %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%6#2) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<49x160xf32>>
          ...
          %15 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%10 : tensor<49x160xf32>) outs(%14 : tensor<49xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %18 = arith.addf %in, %out : f32
            linalg.yield %18 : f32
          } -> tensor<49xf32>
          %16 = linalg.generic {indexing_maps = [#map2, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%11, %15 : tensor<160x49xf32>, tensor<49xf32>) outs(%14 : tensor<49xf32>) attrs =  {lowering_config = #config2} {
          ...
          %17 = linalg.generic {indexing_maps = [#map2, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%11, %15, %16 : tensor<160x49xf32>, tensor<49xf32>, tensor<49xf32>) outs(%12 : tensor<49x160xf32>) attrs =  {lowering_config = #config3} {
  ...
  util.global private mutable @main_graph$async_dispatch_103_embedded_elf_riscv_64_main_graph$async_dispatch_103_reduction_49x160_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_103_embedded_elf_riscv_64_main_graph$async_dispatch_103_reduction_49x160_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_103_embedded_elf_riscv_64_main_graph$async_dispatch_103_reduction_49x160_f32_buffer = util.global.load @main_graph$async_dispatch_103_embedded_elf_riscv_64_main_graph$async_dispatch_103_reduction_49x160_f32_buffer : !hal.buffer
