...
#config2 = #iree_cpu.lowering_config<distribution = [1, 7, 0], vector_common_parallel = [1, 8, 0], vector_reduction = [0, 0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_153 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_153_softmax_8x49x49xf32_dispatch_tensor_store() attributes {translation_info = #translation} {
          ...
          %3 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%2) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<8x49x49xf32>>
          %4 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c1003520) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x49x49xf32>>
          ...
          %8 = linalg.fill {lowering_config = #config} ins(%cst_0 : f32) outs(%7 : tensor<8x49xf32>) -> tensor<8x49xf32>
          %9 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "parallel", "reduction"]} ins(%5 : tensor<8x49x49xf32>) outs(%8 : tensor<8x49xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %13 = arith.mulf %in, %cst_1 : f32
            %14 = arith.maxnumf %13, %out : f32
            linalg.yield %14 : f32
          } -> tensor<8x49xf32>
          %10 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%7 : tensor<8x49xf32>) -> tensor<8x49xf32>
          %11 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "parallel", "reduction"]} ins(%5, %9 : tensor<8x49x49xf32>, tensor<8x49xf32>) outs(%10 : tensor<8x49xf32>) attrs =  {lowering_config = #config2} {
          ...
          %12 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel", "parallel"]} ins(%5, %9, %11 : tensor<8x49x49xf32>, tensor<8x49xf32>, tensor<8x49xf32>) outs(%6 : tensor<8x49x49xf32>) attrs =  {lowering_config = #config3} {
  ...
  util.global private mutable @main_graph$async_dispatch_153_embedded_elf_riscv_64_main_graph$async_dispatch_153_softmax_8x49x49xf32_dispatch_tensor_store_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_153_embedded_elf_riscv_64_main_graph$async_dispatch_153_softmax_8x49x49xf32_dispatch_tensor_store(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_153_embedded_elf_riscv_64_main_graph$async_dispatch_153_softmax_8x49x49xf32_dispatch_tensor_store_buffer = util.global.load @main_graph$async_dispatch_153_embedded_elf_riscv_64_main_graph$async_dispatch_153_softmax_8x49x49xf32_dispatch_tensor_store_buffer : !hal.buffer
