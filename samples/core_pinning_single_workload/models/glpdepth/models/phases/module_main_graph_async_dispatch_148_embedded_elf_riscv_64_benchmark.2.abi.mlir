#config = #iree_cpu.lowering_config<cache_parallel = [1, 7, 32], vector_common_parallel = [1, 7, 1]>
#config1 = #iree_cpu.lowering_config<cache_parallel = [1, 7, 32, 0], distribution = [1, 7, 32, 0], vector_common_parallel = [1, 7, 1, 0], vector_reduction = [0, 0, 0, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_148 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack() attributes {translation_info = #translation} {
          ...
          %6:3 = util.assume.int 
            ...
            : index, index, index
          %7 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%6#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x256xf32>>
          %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%6#1) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<256x8x32xf32>>
          %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%6#2) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<8x7x32x7x1xf32>>
          ...
          %14 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%13 : tensor<8x49x32xf32>) -> tensor<8x49x32xf32>
          %15 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%10, %11 : tensor<49x256xf32>, tensor<256x8x32xf32>) outs(%14 : tensor<8x49x32xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %17 = arith.mulf %in, %in_0 : f32
            %18 = arith.addf %out, %17 : f32
            linalg.yield %18 : f32
          } -> tensor<8x49x32xf32>
          %16 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%15 : tensor<8x49x32xf32>) outs(%13 : tensor<8x49x32xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
            %17 = arith.addf %in, %cst : f32
            linalg.yield %17 : f32
          } -> tensor<8x49x32xf32>
          %pack = linalg.pack %16 outer_dims_perm = [0, 1, 2] inner_dims_pos = [1, 2] inner_tiles = [7, 1] into %12 {lowering_config = #config2} : tensor<8x49x32xf32> -> tensor<8x7x32x7x1xf32>
  ...
  util.global private mutable @main_graph$async_dispatch_148_embedded_elf_riscv_64_main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_148_embedded_elf_riscv_64_main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_148_embedded_elf_riscv_64_main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack_buffer = util.global.load @main_graph$async_dispatch_148_embedded_elf_riscv_64_main_graph$async_dispatch_148_matmul_like_8x49x32x256_f32_pack_buffer : !hal.buffer
