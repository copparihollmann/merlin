#config = #iree_cpu.lowering_config<distribution = [5, 28, 35], vector_common_parallel = [1, 1, 7]>
#config1 = #iree_cpu.lowering_config<vector_common_parallel = [1, 1, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_110 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_110_elementwise_broadcast_5x49x32_f32_pack() attributes {translation_info = #translation} {
          ...
          %4:2 = util.assume.int 
            ...
            : index, index
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%4#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x1x5x49x32xf32>>
          %6 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%4#1) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<5x5x49x7x1xf32>>
          ...
          %10 = linalg.generic {indexing_maps = [#map], iterator_types = ["parallel", "parallel", "parallel"]} outs(%9 : tensor<5x49x32xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%out: f32):
          ...
          %pack = linalg.pack %10 padding_value(%cst : f32) outer_dims_perm = [0, 2, 1] inner_dims_pos = [2, 1] inner_tiles = [7, 1] into %8 {lowering_config = #config1} : tensor<5x49x32xf32> -> tensor<5x5x49x7x1xf32>
