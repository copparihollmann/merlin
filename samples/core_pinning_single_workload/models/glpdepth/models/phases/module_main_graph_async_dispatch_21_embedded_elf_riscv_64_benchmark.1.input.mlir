#config = #iree_cpu.lowering_config<cache_parallel = [64, 8, 56], vector_common_parallel = [1, 8, 32]>
#config1 = #iree_cpu.lowering_config<cache_parallel = [64, 8, 56, 0], distribution = [64, 8, 56, 0], vector_common_parallel = [1, 8, 32, 0], vector_reduction = [0, 0, 0, 1]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_21 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_21_matmul_like_128x56x56x32_f32() attributes {translation_info = #translation} {
          ...
          %3 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1204224) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<56x56x32xf32>>
          %4 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%2) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x128xf32>>
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1605632) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<readwrite:tensor<128x58x58xf32>>
          ...
          %9 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%8 : tensor<128x56x56xf32>) -> tensor<128x56x56xf32>
          %10 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction"]} ins(%6, %7 : tensor<56x56x32xf32>, tensor<32x128xf32>) outs(%9 : tensor<128x56x56xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %12 = arith.mulf %in, %in_0 : f32
            %13 = arith.addf %out, %12 : f32
            linalg.yield %13 : f32
          } -> tensor<128x56x56xf32>
          %11 = linalg.generic {indexing_maps = [#map3, #map3], iterator_types = ["parallel", "parallel", "parallel"]} ins(%10 : tensor<128x56x56xf32>) outs(%8 : tensor<128x56x56xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_21_embedded_elf_riscv_64_main_graph$async_dispatch_21_matmul_like_128x56x56x32_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_21_embedded_elf_riscv_64_main_graph$async_dispatch_21_matmul_like_128x56x56x32_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_21_embedded_elf_riscv_64_main_graph$async_dispatch_21_matmul_like_128x56x56x32_f32_buffer = util.global.load @main_graph$async_dispatch_21_embedded_elf_riscv_64_main_graph$async_dispatch_21_matmul_like_128x56x56x32_f32_buffer : !hal.buffer
