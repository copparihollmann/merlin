...
#config2 = #iree_cpu.lowering_config<distribution = [32, 0], vector_common_parallel = [8, 0], vector_reduction = [0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_12 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_12_softmax_3136x49xf32_dispatch_tensor_store() attributes {translation_info = #translation} {
          ...
          %4:2 = util.assume.int 
            ...
            : index, index
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%4#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3136x49xf32>>
          %6 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%4#1) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x49xf32>>
          ...
          %10 = linalg.fill {lowering_config = #config} ins(%cst_0 : f32) outs(%9 : tensor<3136xf32>) -> tensor<3136xf32>
          %11 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%7 : tensor<3136x49xf32>) outs(%10 : tensor<3136xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %15 = arith.mulf %in, %cst_1 : f32
            %16 = arith.maxnumf %15, %out : f32
            linalg.yield %16 : f32
          } -> tensor<3136xf32>
          %12 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%9 : tensor<3136xf32>) -> tensor<3136xf32>
          %13 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%7, %11 : tensor<3136x49xf32>, tensor<3136xf32>) outs(%12 : tensor<3136xf32>) attrs =  {lowering_config = #config2} {
          ...
          %14 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%7, %11, %13 : tensor<3136x49xf32>, tensor<3136xf32>, tensor<3136xf32>) outs(%8 : tensor<3136x49xf32>) attrs =  {lowering_config = #config3} {
  ...
  util.global private mutable @main_graph$async_dispatch_12_embedded_elf_riscv_64_main_graph$async_dispatch_12_softmax_3136x49xf32_dispatch_tensor_store_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_12_embedded_elf_riscv_64_main_graph$async_dispatch_12_softmax_3136x49xf32_dispatch_tensor_store(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_12_embedded_elf_riscv_64_main_graph$async_dispatch_12_softmax_3136x49xf32_dispatch_tensor_store_buffer = util.global.load @main_graph$async_dispatch_12_embedded_elf_riscv_64_main_graph$async_dispatch_12_softmax_3136x49xf32_dispatch_tensor_store_buffer : !hal.buffer
