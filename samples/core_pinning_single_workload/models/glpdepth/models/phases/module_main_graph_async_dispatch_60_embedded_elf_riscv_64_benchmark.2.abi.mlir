#config = #iree_cpu.lowering_config<distribution = [2, 64, 49], vector_common_parallel = [1, 32, 7]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_60 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_60_unpack_f32() attributes {translation_info = #translation} {
          %c1003520 = arith.constant 1003520 : index
          %c1317120 = arith.constant 1317120 : index
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1003520) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2x7x25x7x32xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c1317120) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2x784x49xf32>>
          ...
          %unpack = linalg.unpack %2 outer_dims_perm = [0, 2, 1] inner_dims_pos = [2, 1] inner_tiles = [7, 32] into %3 {lowering_config = #config} : tensor<2x7x25x7x32xf32> -> tensor<2x784x49xf32>
  ...
  util.global private mutable @main_graph$async_dispatch_60_embedded_elf_riscv_64_main_graph$async_dispatch_60_unpack_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_60_embedded_elf_riscv_64_main_graph$async_dispatch_60_unpack_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_60_embedded_elf_riscv_64_main_graph$async_dispatch_60_unpack_f32_buffer = util.global.load @main_graph$async_dispatch_60_embedded_elf_riscv_64_main_graph$async_dispatch_60_unpack_f32_buffer : !hal.buffer
