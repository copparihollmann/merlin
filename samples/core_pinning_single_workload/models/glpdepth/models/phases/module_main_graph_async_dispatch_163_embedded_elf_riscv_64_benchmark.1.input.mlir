...
#config1 = #iree_cpu.lowering_config<distribution = [7, 7, 32, 0, 0], vector_common_parallel = [1, 8, 1, 0, 0], vector_reduction = [0, 0, 0, 1, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_163 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_163_conv_7x7x1024x3x3_f32() attributes {translation_info = #translation} {
          ...
          %3 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1003520) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1024x9x9xf32>>
          %4 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%2) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<1024x3x3xf32>>
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1335296) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<7x7x1024xf32>>
          ...
          %10 = linalg.fill {lowering_config = #config} ins(%cst_2 : f32) outs(%9 : tensor<1024x7x7xf32>) -> tensor<1024x7x7xf32>
          %11 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "parallel", "reduction", "reduction"]} ins(%6, %7 : tensor<1024x9x9xf32>, tensor<1024x3x3xf32>) outs(%10 : tensor<1024x7x7xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_3: f32, %out: f32):
            %13 = arith.mulf %in, %in_3 : f32
            %14 = arith.addf %out, %13 : f32
            linalg.yield %14 : f32
          } -> tensor<1024x7x7xf32>
          %12 = linalg.generic {indexing_maps = [#map3, #map4], iterator_types = ["parallel", "parallel", "parallel"]} ins(%11 : tensor<1024x7x7xf32>) outs(%8 : tensor<7x7x1024xf32>) attrs =  {lowering_config = #config} {
          ^bb0(%in: f32, %out: f32):
  ...
  util.global private mutable @main_graph$async_dispatch_163_embedded_elf_riscv_64_main_graph$async_dispatch_163_conv_7x7x1024x3x3_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_163_embedded_elf_riscv_64_main_graph$async_dispatch_163_conv_7x7x1024x3x3_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_163_embedded_elf_riscv_64_main_graph$async_dispatch_163_conv_7x7x1024x3x3_f32_buffer = util.global.load @main_graph$async_dispatch_163_embedded_elf_riscv_64_main_graph$async_dispatch_163_conv_7x7x1024x3x3_f32_buffer : !hal.buffer
