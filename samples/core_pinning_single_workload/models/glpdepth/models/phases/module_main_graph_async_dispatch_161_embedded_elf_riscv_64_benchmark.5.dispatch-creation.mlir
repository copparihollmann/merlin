...
#config1 = #iree_cpu.lowering_config<vector_common_parallel = [1, 0], vector_inner_parallel = [0, 8]>
#config2 = #iree_cpu.lowering_config<vector_common_parallel = [1, 0], vector_reduction = [0, 8]>
#config3 = #iree_cpu.lowering_config<distribution = [1, 0], vector_common_parallel = [1, 0], vector_reduction = [0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_161 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_161_reduction_49x256_f32() attributes {translation_info = #translation} {
          ...
          %4:2 = util.assume.int 
            ...
            : index, index
          %5 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%4#0) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x256xf32>>
          %6 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c627200) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<49x256xf32>>
          %7 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%4#1) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<49x256xf32>>
          %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c677376) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<49x256xf32>>
          ...
          %13 = linalg.fill {lowering_config = #config} ins(%cst_1 : f32) outs(%11 : tensor<49xf32>) -> tensor<49xf32>
          %14 = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel", "parallel"]} ins(%9, %10 : tensor<49x256xf32>, tensor<49x256xf32>) outs(%12 : tensor<49x256xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
            %18 = arith.addf %in_2, %cst_1 : f32
            %19 = arith.addf %in, %18 : f32
            linalg.yield %19 : f32
          } -> tensor<49x256xf32>
          %15 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%14 : tensor<49x256xf32>) outs(%13 : tensor<49xf32>) attrs =  {lowering_config = #config2} {
          ^bb0(%in: f32, %out: f32):
            %18 = arith.addf %in, %out : f32
            linalg.yield %18 : f32
          } -> tensor<49xf32>
          %16 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%14, %15 : tensor<49x256xf32>, tensor<49xf32>) outs(%13 : tensor<49xf32>) attrs =  {lowering_config = #config3} {
          ^bb0(%in: f32, %in_2: f32, %out: f32):
          ...
          %17 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%14, %15, %16 : tensor<49x256xf32>, tensor<49xf32>, tensor<49xf32>) outs(%12 : tensor<49x256xf32>) attrs =  {lowering_config = #config1} {
