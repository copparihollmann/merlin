...
#config1 = #iree_cpu.lowering_config<vector_common_parallel = [8, 0], vector_reduction = [0, 8]>
#config2 = #iree_cpu.lowering_config<vector_common_parallel = [8, 0], vector_reduction = [0, 1]>
#config3 = #iree_cpu.lowering_config<vector_common_parallel = [8, 0], vector_inner_parallel = [0, 8]>
#config4 = #iree_cpu.lowering_config<distribution = [32, 0], vector_common_parallel = [8, 0], vector_reduction = [0, 8]>
...
module {
  ...
  hal.executable private @main_graph$async_dispatch_2 {
    hal.executable.variant public @embedded_elf_riscv_64 target(#executable_target_embedded_elf_riscv_64) {
      ...
      builtin.module {
        func.func @main_graph$async_dispatch_2_reduction_3136x32_f32() attributes {translation_info = #translation} {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c1036224) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<3136x32xf32>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c634816) flags("ReadOnly|Indirect") : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x3136xf32>>
          %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x32xf32>>
          %3 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c1437632) flags(Indirect) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<3136x32xf32>>
          ...
          %8 = linalg.fill {lowering_config = #config} ins(%cst_1 : f32) outs(%7 : tensor<3136xf32>) -> tensor<3136xf32>
          %9 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%4 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %15 = arith.addf %in, %out : f32
            linalg.yield %15 : f32
          } -> tensor<3136xf32>
          %10 = linalg.generic {indexing_maps = [#map2, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%5, %9 : tensor<32x3136xf32>, tensor<3136xf32>) outs(%8 : tensor<3136xf32>) attrs =  {lowering_config = #config2} {
          ...
          %11 = linalg.generic {indexing_maps = [#map2, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%5, %9, %10 : tensor<32x3136xf32>, tensor<3136xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) attrs =  {lowering_config = #config3} {
          ...
          %12 = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel", "reduction"]} ins(%11 : tensor<3136x32xf32>) outs(%8 : tensor<3136xf32>) attrs =  {lowering_config = #config1} {
          ^bb0(%in: f32, %out: f32):
            %15 = arith.addf %in, %out : f32
            linalg.yield %15 : f32
          } -> tensor<3136xf32>
          %13 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "reduction"]} ins(%11, %12 : tensor<3136x32xf32>, tensor<3136xf32>) outs(%8 : tensor<3136xf32>) attrs =  {lowering_config = #config4} {
          ^bb0(%in: f32, %in_3: f32, %out: f32):
            %15 = arith.divf %in_3, %cst : f32
            %16 = arith.subf %in, %15 : f32
            %17 = arith.mulf %16, %16 : f32
          ...
          %14 = linalg.generic {indexing_maps = [#map, #map1, #map1, #map], iterator_types = ["parallel", "parallel"]} ins(%11, %12, %13 : tensor<3136x32xf32>, tensor<3136xf32>, tensor<3136xf32>) outs(%6 : tensor<3136x32xf32>) attrs =  {lowering_config = #config3} {
          ^bb0(%in: f32, %in_3: f32, %in_4: f32, %out: f32):
            %15 = arith.divf %in_3, %cst : f32
            %16 = arith.subf %in, %15 : f32
            %17 = arith.divf %in_4, %cst : f32
  ...
  util.global private mutable @main_graph$async_dispatch_2_embedded_elf_riscv_64_main_graph$async_dispatch_2_reduction_3136x32_f32_buffer : !hal.buffer
  util.initializer {
  ...
  util.func public @main_graph$async_dispatch_2_embedded_elf_riscv_64_main_graph$async_dispatch_2_reduction_3136x32_f32(%arg0: i32) attributes {iree.abi.stub, iree.reflection = {iree.benchmark = "dispatch"}} {
    ...
    %main_graph$async_dispatch_2_embedded_elf_riscv_64_main_graph$async_dispatch_2_reduction_3136x32_f32_buffer = util.global.load @main_graph$async_dispatch_2_embedded_elf_riscv_64_main_graph$async_dispatch_2_reduction_3136x32_f32_buffer : !hal.buffer
